  ...
  it('not.value assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.match(err.message, new RegExp(`Testing if value of element <.test_element> doesn't equal 'some-value' in 5ms - expected "not equals 'some-value'" but got: "some-value" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });
  ...
  it('value assertion - element not found', function() {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.match(err.message, new RegExp(`Test value of element <.test_element> == 'some-value' in 5ms - expected "equals 'some-value'" but got: "element could not be located" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });

  ...
  it('not.valueContains assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.match(err.message, new RegExp(`Testing if value of element <.test_element> doesn't contain 'some-value' in 5ms - expected "not contains 'some-value'" but got: "some-value" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });
  ...
  it('valueContains assertion - element not found', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.match(err.message, new RegExp(`Test value of element "<.test_element>" contains 'some-value' in 5ms - expected "contains 'some-value'" but got: "element could not be located" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });

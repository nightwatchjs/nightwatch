  ...
  it('not.attributeContains assertion failed', function() {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.strictEqual(err.message, 'Error while running "attributeContains" command: Testing if attribute \'href\' of element <.test_element> doesn\'t contain \'google\' in 5ms - expected "not contains \'google\'" but got: "http://google.org"');
      }
    });
  });
  ...
  it('attributeContains assertion - element not found', function() {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.strictEqual(err.message, 'Error while running "attributeContains" command: Testing if attribute \'role\' of element <.test_element> contains \'main\' in 5ms - expected "contains \'main\'" but got: "element could not be located"');
      }
    });
  });
  ...
  it('attributeContains assertion - attribute not found', function() {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.strictEqual(err.message, 'Error while running "attributeContains" command: Testing if attribute \'role\' of element <.test_element> contains \'main\' in 5ms - expected "contains \'main\'" but got: "Element does not have a \'role\' attribute"');
      }
    });
  });

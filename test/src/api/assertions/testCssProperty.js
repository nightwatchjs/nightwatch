  ...
  it('not.cssProperty assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.match(err.message, new RegExp(`Testing if element <.test_element> doesn't have css property 'display: none' in 5ms - expected "not none" but got: "none" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });
  ...
  it('cssProperty assertion - element not found', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.match(err.message, new RegExp(`Test attribute <.test_element> from element "'display" == none' in 5ms - expected "none" but got: "element could not be located" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });

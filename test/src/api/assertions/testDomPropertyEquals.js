  ...
  it('not.domPropertyEquals assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.match(err.message, new RegExp(`Testing if dom property 'className' of element <.test_element> doesn't equal 'visible-element' in 5ms - expected "not visible-element" but got: "visible-element" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });
  ...
  it('domPropertyEquals assertion - element not found', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.match(err.message, new RegExp(`Test attribute 'className' from element "<.test_element>" == 'visible-element' in 5ms - expected "visible-element" but got: "element could not be located" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });

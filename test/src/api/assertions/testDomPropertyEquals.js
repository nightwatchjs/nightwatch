  ...
  it('not.domPropertyEquals assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.strictEqual(err.message, `Error while running "assert.domPropertyEquals" command: [NightwatchAssertError] Testing if dom property 'className' of element <.test_element> doesn't equal 'visible-element' in 5ms - expected "not visible-element" but got: "visible-element" (${instance.elapsedTime}ms)`);
      }
    });
  });
  ...
  it('domPropertyEquals assertion - element not found', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.strictEqual(err.message, `Error while running "assert.domPropertyEquals" command: [NightwatchAssertError] Test attribute 'className' from element "<.test_element>" == 'visible-element' in 5ms - expected "visible-element" but got: "element could not be located" (${instance.elapsedTime}ms)`);
      }
    });
  });

  ...
  it('title assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.match(err.message, new RegExp(`Testing if the page title equals 'Test Title' in 5ms - expected "is 'Test Title'" but got: "Wrong Title" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });

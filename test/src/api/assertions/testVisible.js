  ...
  it('.not.visible assertion failed', function () {
    return assertionTest({
      ...
      assertion({reporter, instance, queueOpts, err}) {
        ...
        assert.match(err.message, new RegExp(`Testing if element <.test_element> is not visible in 5ms - expected "is not visible" but got: "visible" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });
  ...
  it('visible assertion not - element not found', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.match(err.message, new RegExp(`Testing if element <.test_element> is visible in 5ms - expected "is visible" but got: "element could not be located" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });
});

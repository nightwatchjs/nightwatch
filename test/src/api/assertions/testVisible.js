  ...
  it('.not.visible assertion failed', function () {
    return assertionTest({
      ...
      assertion({reporter, instance, queueOpts, err}) {
        ...
        assert.strictEqual(err.message, 'Error while running "visible" command: Testing if element <.test_element> is not visible in 5ms - expected "is not visible" but got: "visible"');
      }
    });
  });
  ...
  it('visible assertion not - element not found', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.strictEqual(err.message, 'Error while running "visible" command: Testing if element <.test_element> is visible in 5ms - expected "is visible" but got: "element could not be located"');
      }
    });
  });
});

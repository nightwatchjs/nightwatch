  ...
  it('not.attributeEquals assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.strictEqual(err.message, 'Error while running "attributeEquals" command: Testing if attribute \'role\' of element <.test_element> doesn\'t equal \'main\' in 5ms - expected "not equals \'main\'" but got: "main"');
      }
    });
  });
  ...
  it('attributeEquals assertion - element not found', function() {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.strictEqual(err.message, 'Error while running "attributeEquals" command: Test attribute \'role\' from element "<.test_element>" == \'main\' in 5ms - expected "equals \'main\'" but got: "element could not be located"');
      }
    });
  });
  ...
  it('attributeEquals assertion - attribute not found', function() {
    return assertionTest({
      ...
      assertion({instance, message, failure}) {
        assert.strictEqual(message, 'Testing if attribute \'role\' of element <.test_element> equals \'main\' in 5ms - expected "equals \'main\'" but got: "Element does not have a \'role\' attribute"');
        assert.strictEqual(instance.hasFailure(), false);
        assert.strictEqual(instance.getValue(), null);

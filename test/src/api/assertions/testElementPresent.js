  ...
  it('not.elementPresent assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.strictEqual(err.message, 'Error while running "elementPresent" command: Testing if element <.test_element> is not present in 5ms - expected "is not present" but got: "present"');
      }
    });
  });

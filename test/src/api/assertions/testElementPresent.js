  ...
  it('not.elementPresent assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.match(err.message, new RegExp(`Testing if element <.test_element> is not present in 5ms - expected "is not present" but got: "present" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });

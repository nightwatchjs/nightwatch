  ...
  it('hidden assertion - element not found', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.match(err.message, new RegExp(`Testing if element \\[ '.test_element' \\] is hidden. Element could not be located in 5ms - expected "true" but got: "" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });
});

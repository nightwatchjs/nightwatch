  ...
  it('hidden assertion - element not found', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.strictEqual(err.message, 'Error while running "hidden" command: Testing if element .test_element is hidden. Element could not be located in 5ms - expected "true" but got: ""');
      }
    });
  });
});

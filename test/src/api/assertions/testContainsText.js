  ...
  it('not.containsText assertion failed', function () {
    return assertionTest({
      ...
      assertion({instance, queueOpts, err}) {
        ...
        assert.match(err.message, new RegExp(`Test message in 5ms - expected "does not contain text 'text result'" but got: "contains 'text result'" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });
  ...
  it('containsText assertion - element not found', function () {
    return assertionTest({
      ...
      assertion({instance, failure, err}) {
        ...
        assert.match(err.message, new RegExp(`Test message in 5ms - expected "contains text 'text result'" but got: "element could not be located" \\(${instance.elapsedTime}ms\\)`));
      }
    });
  });
});

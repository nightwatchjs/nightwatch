...
describe('element().getAttribute() command', function () {
  ...
  it('test .element().find().attr() assert', async function(){
    MockServer.addMock({
      url: '/session/13521-10219-202/execute/sync',
      method: 'POST',
      response: JSON.stringify({
  ...
  it('test .element.find().attribute() assert', async function() {
    MockServer.addMock({
      url: '/session/13521-10219-202/execute/sync',
      method: 'POST',
      response: JSON.stringify({
    ...
    const resultPromise = this.client.api.element.find('#signupSection').attribute('type');
    assert.strictEqual(resultPromise instanceof Element, false);
    assert.strictEqual(typeof resultPromise.find, 'undefined');

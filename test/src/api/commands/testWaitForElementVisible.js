...
describe('waitForElementVisible', function() {
  ...
  it('client.waitForElementVisible() with element not found but suppressNotFoundErrors is true', function () {
    let result;

    this.client.api.globals.waitForConditionPollInterval = 5;
    this.client.api.waitForElementVisible({selector: '.weblogin', timeout: 11, suppressNotFoundErrors: true}, function (res, instance) {
      commandInstance = instance;
      result = res;
    });

...
class Utils {
  ...
  static getSdkRootFromEnv() {
    const androidHomeInGlobalEnv = 'ANDROID_HOME' in process.env;
    ...
    if (androidHome === undefined) {
      ...
      err = new Error(`ANDROID_HOME is set to '${androidHome}'${fromDotEnv} which is NOT a valid path!`);
    }
    
    err.help = [
      `Run command to setup ${Logger.colors.cyan('npx nightwatch-mobile-helper android --setup')}`,
      `Run command for more help ${Logger.colors.cyan('npx nightwatch-mobile-helper android --help \n')}`
    ];

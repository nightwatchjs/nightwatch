...
class Utils {
  ...
  static isRerunFailedEnabled(argv = {}) {
    if (process.env.NIGHTWATCH_RERUN_FAILED === 'true') {
      return true;
    }

    if (argv['rerun-failed']) {
      return true;
    }
  ...
  static getRerunFailedFile() {
    if (process.env.NIGHTWATCH_RERUN_FAILED_FILE) {
      return process.env.NIGHTWATCH_RERUN_FAILED_FILE;
    }

...
class Utils {
  ...
  /**
   * Collects all unquoted arguments in an array of arguments and returns them.
   * e.g.: [--argument="Hello there"] will be okay.
   * e.g.: [--argument=Hello there] will be collected because it contains an unquoted whitespace.
   * @param args
   * @returns {Array}
   */
  static findUnquotedCliArgumentsWithWhitespaces(args) {
    const isQuotedRegExp = RegExp('^[\'"]');
    const containsWhitespaceRegExp = RegExp(' ');
    const unquotedArgumentsWithWhitespaces = [];

    if (Array.isArray(args)) {
      args.map((cliArgument) => {
        let argumentTokens = cliArgument.split('=');
        let valueToken = argumentTokens[1];
        if (Utils.isString(valueToken)) {
          if (!isQuotedRegExp.test(valueToken) && containsWhitespaceRegExp.test(valueToken)) {
            unquotedArgumentsWithWhitespaces.push(cliArgument);
          }
        }
      });

      return unquotedArgumentsWithWhitespaces;
    }
  }

  /**
   * Check if the chromeOptions in the base Settings are valid.
   * @param baseSettings
   */
  static validateChromeOptions(baseSettings) {

    let invalidChromeArguments;

    if (baseSettings && baseSettings.test_settings) {
      Object.keys(baseSettings.test_settings).map((testSettingKey) => {
        let testSetting = baseSettings.test_settings[testSettingKey];
        if (Utils.isObject(testSetting.desiredCapabilities) &&
            Utils.isObject(testSetting.desiredCapabilities.chromeOptions) &&
            Utils.isObject(testSetting.desiredCapabilities.chromeOptions.args)) {

          invalidChromeArguments = Utils.findUnquotedCliArgumentsWithWhitespaces(testSetting.desiredCapabilities.chromeOptions.args);
        }
      });
    }

    return invalidChromeArguments;
  };

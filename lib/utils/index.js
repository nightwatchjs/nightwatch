...
class Utils {
  ...
  static getRerunFailedFile() {
    const jsonFile = path.resolve(process.env.NIGHTWATCH_RERUN_FAILED_FILE || '');
    
    if (!Utils.fileExistsSync(jsonFile)) {
      const err = new Error('Unable to find the Json reporter file to rerun failed tests');

      err.showTrace = false;
      err.detailedErr = 'Configure the environment variable NIGHTWATCH_RERUN_FAILED_TEST with Json reporter file path';
      err.help = [Logger.colors.cyan('export NIGHTWATCH_RERUN_FAILED_TEST="JSON-REPORTER-PATH"')];

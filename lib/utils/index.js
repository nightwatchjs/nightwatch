...
class Utils {
  ...
  static copyDir(srcDir, destDir, excludeDir) {
    ...
    for (const file of fs.readdirSync(srcDir)) {
      ...
      Utils.copy(srcFile, destFile, excludeDir);
    }
  };

  static copy(src, dest, excludeDir = [], overwrite = false) {
    const stat = fs.statSync(src);
    if (stat.isDirectory()) {
      return Utils.copyDir(src, dest, excludeDir);
    }

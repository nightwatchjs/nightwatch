...
class Logger {
  ...
  getFailedAssertions(assertions, modulepath) {
    return assertions.reduce((prev, a) => {
      if (a.failure !== false) {
        prev.push(this.getAssertionErrorContent(a, modulepath));
      }
  ...
  getErrorContent(error, modulepath) {
    ...
    const errorObj = Errors.getErrorObject(error);
    ...
    if (error.stack){
      const stack = '\tat' + error.stack.split(/ {4}at/g).slice(1).join('\tat');

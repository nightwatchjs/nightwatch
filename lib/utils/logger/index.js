...
class Logger {
  ...
  getAssertionErrorContent(err) {
    ...
    content.push(`  ${colors.light_red(`→ ${err.name}`)}`);
    content.push(`\n    ${colors.red(err.fullMsg || err.message)}`);
  
    stack = 'at DescribeInstance.<anonymous>' + stack.split('at DescribeInstance.<anonymous>').pop();
    content.push(colors.brown('\n    Error location :'));
    content.push(beautifyStackTrace(stack, true));
  
    content.push(colors.brown('    Stack Trace :'));
    content.push(colors.stack_trace(`\t${stack.replace(/ {4}/g, '\t')}\n`));
  ...
  getErrorContent(error) {
    ...
    let errorObj = Errors.getErrorObject(error);
    let content = [];
    
    content.push(`  ${colors.light_red(`→ ${error.name}`)}`);
    ...
    const stack = 'at DescribeInstance.<anonymous>' + error.stack.split('at DescribeInstance.<anonymous>').pop();
    content.push(colors.brown('\n    Error location :'));
    content.push(beautifyStackTrace(stack, true));

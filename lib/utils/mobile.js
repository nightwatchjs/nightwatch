...
function requireMobileHelper() {
  try {
    ...
    if (err.code === 'MODULE_NOT_FOUND') {
      const error = new Error(`@nightwatch/mobile-helper needs to be installed as a project dependency. You can install @nightwatch/mobile-helper from NPM using:\n\n        ${Logger.colors.light_green('npm i @nightwatch/mobile-helper --save-dev')}`);
...
class AndroidHomeError extends Error {
  ...
  get message() {
    ...
    if (this.androidHome === undefined) {
      msg = 'ANDROID_HOME environment variable is NOT set or Not a valid path!';
    } else {
      msg = `ANDROID_HOME is set to '${this.androidHome}' which is NOT a valid path!`;
    }
...
class AndroidBinaryError extends Error {
  ...
  get binaryLocation() {
    ...
    return mobileHelper.getBinaryLocation(getSdkRootFromEnv(), mobileHelper.getPlatformName(), this.binaryName, true);
  }
  ...
  get help() {
    ...
    if (this.binaryLocation === '') {
      ...
      help = [
        ...
        `If it still doesn't work, start the emulator by yourself by running ${Logger.colors.cyan(this.binaryLocation + ' @' + this.avd)} and then run the test.`
      ];
    }
...
class AndroidConnectionError extends Error {
  ...
  get help() {
    const adbError = new AndroidBinaryError(this.message, 'adb');

    let binaryLocation = adbError.binaryLocation();
    let help;

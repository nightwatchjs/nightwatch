...
class IosSessionNotCreatedError extends Error {
  ...
  get help() {
    ...
    if (this.message.includes('session timed out')) {
      ...
      help = [
        `Verify the UDID of the device set in Nightwatch configuration (look for safari:deviceUDID desired capability) or pass the UDID using ${Logger.colors.cyan('--deviceId')} flag in the test command.`,
        'Re-run the test command',
        `If it doesn't work, try running: ${Logger.colors.cyan('npx run @nightwatch/mobile-helper ios')}`
      ];
      ...
      if (isRealIos(this.desiredCapabilities)) {
        help = [
          `Run command to check device connection, ${Logger.colors.cyan('system_profiler SPUSBDataType | sed -n \'/iPhone/,/Serial/p\' | grep \'Serial Number:\' | awk -F \': \' \'{print $2}')}`,
          ...help
        ];
      }
    }

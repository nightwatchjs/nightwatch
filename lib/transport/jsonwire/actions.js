...
module.exports = {
  ...
  session: {
    ...
    switchToFrame(frameId) {
      ...
      if (!(typeof frameId === 'number' || frameId.ELEMENT)) {
        let errBody = frameId;
        if (typeof frameId === 'object') {
          errBody = JSON.stringify(frameId) + ' missing { ELEMENT : ... }'
        }
        Logger.error(`frameId must represent a Number or web element : ${errBody}`);  
        Logger.error('See https://www.w3.org/TR/webdriver/#switch-to-frame');
      }
      return TransportActions.post({
        path: '/frame',
        data: {

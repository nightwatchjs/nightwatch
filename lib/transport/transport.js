...
class Transport extends EventEmitter {
  ...
  static adaptWebdriverSettings(settings, usingSeleniumServer = false) {
    let browserName = Transport.getBrowserName(settings);
    ...
    switch (browserName) {
      ...
      case BrowserName.CHROME: {
        const {chromeOptions = {}, 'goog:chromeOptions': googChromeOpts = {}} = settings.desiredCapabilities;
        const {edgeOptions = {}, 'ms:edgeOptions': msEdgeOpts = {}} = settings.desiredCapabilities;

        if (chromeOptions.w3c || googChromeOpts.w3c || msEdgeOpts.w3c || edgeOptions.w3c) {
          settings.webdriver.use_legacy_jsonwire = false;
        }
      }
    }

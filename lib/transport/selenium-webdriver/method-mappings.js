  ...
  get methods() {
    return {
      ...
      session: {
        ...
        async isElementPresent(webElementOrShadowRoot) {
          let element = await this.getWebElement(webElementOrShadowRoot);
          if (element){
            return true;
          } 

          element=await this.getShadowRoot(webElementOrShadowRoot);
          
          return element && element instanceof ShadowRoot;
        },

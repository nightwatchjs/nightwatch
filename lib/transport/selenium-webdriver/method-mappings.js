    ...
    return {
      ...
      session: {
        ...
        async clearElementValue(webElementOrId) {
          const element = this.getWebElement(webElementOrId);
          await element.clear();
          // Check if the element still has some value after clearing
          const value = await element.getAttribute('value');
          if (value && value.length > 0) {
            // Construct a string of backspaces to clear the element's value
            const backspaces = 'Backspace'.repeat(value.length);
            // Send the backspaces to the element in a single call

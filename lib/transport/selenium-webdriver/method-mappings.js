  ...
  get methods() {
    return {
      ...
      session: {
        ...
        async setElementProperty(webElement, propertyName, value) {
          const element = this.getWebElement(webElement);
          //const elementValue = await element.getProperty(propertyName);

          const fn = function(element, propertyName) {
            return [element.innerText, propertyName];
          }

          const elementId = await element.getId();
          console.log('ELEMENT ID', elementId)
          const result = await this.driver.executeScript('var passedArgs = Array.prototype.slice.call(arguments,0); return (' +
            fn.toString() + ').apply(window, passedArgs);', [
            {[this.transport.elementKey]: elementId}, propertyName, value
          ]);
        ...
        async sendKeysToElement(webElement, value) {
          if (Array.isArray(value)) {
            value = value.join('');
          } else {
            value = String(value);

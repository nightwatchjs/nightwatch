const {WebElement, WebDriver, Origin, By, until, Condition, Key, Actions} = require('selenium-webdriver');
const Pointer = require('selenium-webdriver/lib/input').Pointer;
  ...
  get methods() {
    return {
      ...
      session: {
        ...
        async scrollPage(direction, steps) {
          let scrollKey = null;
          switch (direction) {
            case 'down': scrollKey = Key.PAGE_DOWN; break;
            case 'up': scrollKey = Key.PAGE_UP; break;
          }
          await this.driver.actions({async: true}).sendKeys(scrollKey.repeat(steps)).perform();

          return null;
        },

        async tap(webElementOrId) {
          const pointer1 = new Pointer('finger-1', 'touch');
          const element = await this.getWebElement(webElementOrId);

          // TODO: check for element not present.
          const {height, width, x, y} = await element.getRect();
          const centerX = x + width / 2;
          const centerY = y + height / 2;

          await this.driver.actions({async: true}).insert(
            pointer1,
            pointer1.move({x: centerX, y: centerY}),
            pointer1.press(),
            pointer1.release()
          ).perform();
        ...
        async doubleTap(webElementOrId, wait = 500) {
          const pointer1 = new Pointer('finger-1', 'touch');
          const element = await this.getWebElement(webElementOrId);
        ...
        async longPress(webElementOrId, duration) {
          const pointer1 = new Pointer('finger-1', 'touch');
          const element = await this.getWebElement(webElementOrId);
          ...
          await this.driver.actions({async: true}).insert(
            ...
            pointer1.move({x: centerX, y: centerY}),
            pointer1.press()
          ).pause(duration).insert(
            pointer1,
            pointer1.release()
          ).perform();
        },

        async flick(webElementOrId) {
          return null;
        },

const {WebElement, Origin} = require('selenium-webdriver');
    ...
    return {
      ...
      session: {
        ...
        async dragElement(source, destination) {
          const srcElement = await this.getWebElement(source);
          try {
            const destElement = await this.getWebElement(destination);

            await  this.driver.actions({async: true}).dragAndDrop(srcElement, destElement).perform();
          } catch (err) {
            await  this.driver.actions({async: true}).dragAndDrop(srcElement, destination).perform();
          }

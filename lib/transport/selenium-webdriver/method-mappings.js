    ...
    return {
      ...
      session: {
        ...
        async clickElement(elementSelector) {
          const session = await this.transport.driver.getSession();

          const opts = {
            path: '/session/nightwatch/findElementAndClick',
            method: 'POST',
            port: 3000,
            data: {
          ...
          const result =  await this.transport.sendHttpRequest(opts);
          ...
          if (result.error) {
            return {
              ...
              value: [],
              error: result.error,
              message: `Unable to locate element: ${elementSelector}`
            };
          }

    ...
    return {
      ...
      session: {
        ...
        async startLogsCapture(userCallback) {
          const pageCdpConnection = await this.driver.createCDPConnection('page');
          await this.driver.onLogEvent(pageCdpConnection, userCallback);
        ...
        async catchJsExceptions(userCallback) {
          const pageCdpConnection = await this.driver.createCDPConnection('page');
          await this.driver.onLogException(pageCdpConnection, userCallback);
        ...
        async interceptNetworkCalls(userCallback) {
          ...
          pageCdpConnection._wsConnection.on('message', (message) => {
          ...
          pageCdpConnection._wsConnection.on('message', (message) => {
            ...
            if (params.method === 'Fetch.requestPaused') {
              ...
              if (requestPausedParams.request.url === urlToIntercept) {
                pageCdpConnection.execute('Fetch.fulfillRequest', {
                ...
                pageCdpConnection.execute('Fetch.continueRequest', {
                  requestId: requestPausedParams['requestId']
                });
              }
            }

...
const {Logger, createFolder, isSimulator, isRealIos} = require('../../../utils');
...
class BaseService {
  ...
  onExit(code) {
    ...
    if (isSimulator(this.settings.desiredCapabilities) || isRealIos(this.settings.desiredCapabilities)) {
      let err;

      if (isSimulator(this.settings.desiredCapabilities)) {
        err = new Error('Failed to launch simulator');
      }

      if (isRealIos(this.settings.desiredCapabilities)) {
        err = new Error('Failed to run test on real ios device');
      }
      
      err.help = [
        `Try running command: ${Logger.colors.cyan('npx @nightwatch/mobile-helper ios')} \n`
      ];

...
class AppiumServiceBuilder extends BaseService {
  ...
  async createService(opts = {}) {
    ...
    // if a driver binary is defined, add it to the process args
    // even if the start_process is false, in order for the
    // Options object to be able to locate the driver
    if (this.settings.webdriver.server_path) {
    options.args.unshift(this.settings.webdriver.server_path);
    }

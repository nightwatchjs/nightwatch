...
class SafariServiceBuilder extends BaseDriver {
  ...
  async createService(options) {
    if (useTechnologyPreview(options.get(OPTIONS_CAPABILITY_KEY))) {
      this.settings.server_path = SAFARIDRIVER_TECHNOLOGY_PREVIEW_EXE;
    }

    this.service = new safari.ServiceBuilder(this.settings.server_path);

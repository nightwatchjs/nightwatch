...
class BaseDriver {
  ...
  constructor(settings) {
    ...
    this.settings.host = this.settings.host || BaseDriver.DEFAULT_HOST;
    ...
    const req = http.get({
      ...
      response.on('end', () => {
        if (response.statusCode === 200 || response.statusCode === 404) {
          ...
          Logger.info(`${this.serviceName} up and running on port ${this.settings.port} (${elapsedTime}ms).`);
  ...
  createService() {
    ...
    const {port, host, default_path_prefix, server_path} = this.settings;

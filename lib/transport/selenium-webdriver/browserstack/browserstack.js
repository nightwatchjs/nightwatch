  ...
  async testSuiteFinished(err) {
    try {
      if (this.sessionId) {
        ...
        if (this.productNamespace === 'grid'){
          // eslint-disable-next-line no-console
          console.log('\n  ' + 'See more info, video, & screenshots on Browserstack:\n' +
            '  ' + Logger.colors.light_cyan(`https://${this.productNamespace}.browserstack.com/dashboard/builds/${this.buildId}/sessions/${this.sessionId}`));
        } else {
          // eslint-disable-next-line no-console
          console.log('\n  ' + 'See more info, video, & screenshots on Browserstack:\n' +

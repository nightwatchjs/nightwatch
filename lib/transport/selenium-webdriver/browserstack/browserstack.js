...
class Browserstack extends AppiumBaseServer {
  ...
  findBuildHashId(builds, buildName) {
    const currentBuild = builds.find((item) => item.automation_build?.name === buildName);
    if (currentBuild) {
      return currentBuild.automation_build?.hashed_id;
    }

    return null; // Return null if no matching build is found
  }

  async fetchBuildId(url, buildName, nameAccessor, hashIdAccessor) {
    try {
      let offset = 0;
      let currentBuild;
      ...
      while (true) {
        const buildsResponse = await this.sendHttpRequest({
          url: `${url}?status=running&limit=20&offset=${offset}`,
          method: 'GET',
          use_ssl: true,
          port: 443,
          auth: {
        ...
        const builds = buildsResponse.builds || buildsResponse; // Handle different response structures
        if (!builds || builds.length === 0) {
          break;
        }

        const buildHashId = this.findBuildHashId(builds, buildName);
        if (buildHashId) {

          return buildHashId; // Return the hashed_id
        }
  ...
  async getBuildId() {
    try {
      return await this.fetchBuildId(
        `${this.ApiUrl}/builds.json`,
        this.build,
        (item) => item.automation_build.name, // Accessor for the name property
        (item) => item.automation_build.hashed_id // Accessor for the hashed_id property
      );
    } catch (err) {
      console.error(err);
    }

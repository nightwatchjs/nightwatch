...
class SeleniumWebdriver extends Transport {
  ...
  async createSession(argv) {
    ...
    this.showConnectSpinner(`Connecting to ${host} on port ${port}...\n`);
    ...
    this.showConnectSpinner(`Connected to ${colors.stack_trace(host)} on port ${colors.stack_trace(port)} ${colors.stack_trace('(' + (new Date() - startTime) + 'ms)')}.`);
    if (this.outputEnabled) {

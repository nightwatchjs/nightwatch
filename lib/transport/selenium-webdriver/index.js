...
class Transport extends BaseTransport {
  ...
  async createDriverService({options, moduleKey}) {
    try {
      this.driverService = (Transport.driverService && !Transport.driverService.stopped) ? Transport.driverService : new this.ServiceBuilder(this.settings);
  ...
  async getDriver({options}) {
    if (Transport.driver && !needNewDriver) {
    ...
    needNewDriver = false;
    Transport.driver.session_.catch((err)=> {
      needNewDriver = true;
      throw err;
    });
  ...
  async createSession({argv, moduleKey}) {
    ...
    try {
      this.driver = await this.getDriver({options});

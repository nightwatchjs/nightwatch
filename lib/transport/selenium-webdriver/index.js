...
class Transport extends BaseTransport {
  ...
  async showConnectInfo({startTime, port, host, start_process, sessionInfo}) {
    ...
    if (this.outputEnabled) {
      ...
      console.info(`  Using: ${colors.light_blue(browserName ? browserName: this.browserName)} ${browserVersion ? colors.brown('(' + (browserVersion) + ')'): ''} on ${colors.cyan(platform.toUpperCase() + (platformVersion ? (' (' + platformVersion + ')') : ''))}.\n`);
    }
  }

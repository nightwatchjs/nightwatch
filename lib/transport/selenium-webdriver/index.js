  ...
  handleConnectError(err) {
    ...
    if (!err.detailedErr && this.driverService) {
      ...
      if (err.message.includes('Failed to run adb command') || err.message.includes('no devices online')) {
        const androidServer = new AndroidEmulator();
        const binaryLocation = androidServer.binaryLocation(this.sdkRoot, this.platfrom, 'adb', true);

        let help;
        if (binaryLocation === '') {
          help = [
            `${Logger.colors.cyan('adb')} binary not found. Run command ${Logger.colors.cyan('npx @nightwatch/mobile-helper android')} to setup`
          ];
        } else if (binaryLocation === 'PATH') {
            help = [
              `Run command: ${colors.cyan('adb start-server')}`,
          ...
          if (err.message.includes('Failed to run adb command')) {
            help = [
              `Run command: ${colors.cyan(binaryLocation + ' start-server')}`,
              `If still doesn't work, run "${colors.green('npx @nightwatch/mobile-helper android')}"`
            ];
          }

          if (err.message.includes('no devices online')) {
            help =  [
              `If testing on real-device, check if device is connected with USB debugging turned on and ${colors.cyan(binaryLocation + ' devices')} should list the connected device.`,
              `If testing on emulator, check the configuration to make sure ${colors.cyan('real_mobile')} is set to ${colors.cyan('false')} and ${colors.cyan('avd')} to the AVD to test on.`
            ];
          }
        }
      ...
      if (err.name === 'SessionNotCreatedError' && this.api.isIOS() && this.api.isIOS()) {
        ...
        if (err.message.includes('session timed out')) {
          ...
          help = [
            ...
            `Try run to setup: ${Logger.colors.cyan('npx run @nightwatch/mobile-helper ios')}`
          ];
        } else if (err.message.includes('not find any session hosts') || err.message.includes('Some devices were found')) {

  ...
  async createSession({argv, moduleKey, reuseBrowser = false}) {
    ...
    try {
      ...
      if (this.settings.webdriver.debug_timing) {
        ...
        Logger.info(`[Timing] Breakdown: options=${timing.createOptions}ms, service=${timing.driverService || 0}ms, driver=${timing.driverCreation}ms, export=${timing.sessionExport}ms`);
      }

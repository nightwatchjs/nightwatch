...
class Browserstack extends Selenium {
  ...
  async startLocalTesting() {
    ...
    return new Promise((resolve) => {
      this.bsLocal.start({key: this.settings.desiredCapabilities['browserstack.key']}, () => {
        console.log('BrowserStackLocal running:', this.bsLocal.isRunning());
        resolve();
      });
    })
  }

  async stopLocalTesting() {
    if (!this.useLocal) {
      return;
    }

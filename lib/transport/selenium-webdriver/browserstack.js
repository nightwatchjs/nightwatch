...
class Browserstack extends BaseDriver {
  ...
  get accessKey() {
    return this.settings.desiredCapabilities['bstack:options'].accessKey;
  }

  get username() {
    return this.settings.desiredCapabilities['bstack:options'].userName;
  }

  get build() {
    return this.settings.desiredCapabilities['bstack:options'].buildName;
  }

  get local() {
    return this.settings.desiredCapabilities['bstack:options'].local;
  }
  ...
  constructor(nightwatchInstance, browserName) {
    ...
    this.adaptSettings();
    this.verifySettings();
    this.isMobile = (typeof nightwatchInstance.client.api.isMobile === 'function' &&  nightwatchInstance.client.api.isMobile()) ? true : false;
    this.useLocal = false;

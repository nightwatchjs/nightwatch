...
class Browserstack extends Selenium3 {
  ...
  async testSuiteFinished(hasFailures, results) {
    try {
      ...
      await request(`${Browserstack.ApiUrl}/sessions/${this.sessionId}.json`, {
        ...
        body: {
          ...
          reason: hasFailures ? this.collectFailedTests(results) : ''
        },
        auth: {
          user: this.username,
          pass: this.accessKey,
  ...
  collectFailedTests(results = {}) {
    return Object.entries(results.completed || {})
      .reduce((res, [testCaseName, { errors, failed }]) => {
        if (errors || failed) {

...
function logMessage(type, message, args, alwaysShow) {
  ...
  switch (type) {
    case 'ERROR':
      messageStr = `\n${colors.yellow(type, colors.background.black)} ${timestamp} ${colors.light_red(message)}`;
      break;
...
exports.error = function(message) {
  var args = Array.prototype.slice.call(arguments, 1);
  logMessage('ERROR', message, args, true);
};
...
exports.setOutputEnabled = function(val) {
  Settings.outputEnabled = val;
};

exports.isOutputEnabled = function() {
  return Settings.outputEnabled;
};

exports.setDetailedOutput = function(val) {
  Settings.detailedOutput = val;
};

exports.isDetailedOutput = function() {
...
exports.logDetailedMessage = function(message) {

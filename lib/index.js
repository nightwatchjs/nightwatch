...
class Nightwatch extends events.EventEmitter {
  ...
  setHttpOptions() {
    ...
    if (webdriverOpts.username) {
      this.setApiOption('username', webdriverOpts.username).setApiOption('accessKey', webdriverOpts.access_key);
    ...
    HttpRequest.globalSettings = this.httpOpts;
  ...
  createSession() {
    let protocol = new SeleniumProtocol(this.desiredCapabilities);

    this.session.setTransportProtocol(protocol);

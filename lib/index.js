...
Exports.client = function(settings) {
  return new Nightwatch(settings);
};

Exports.cli = function(callback) {
...
Exports.Reporter = require('./core/reporter.js');
...
Exports.initClient = function(opts = {}) {
  const cliRunner = Exports.CliRunner();
  cliRunner.initTestSettings(opts);

  const settings = cliRunner.settings;
  const client = Exports.client(settings);
  const reporter = new Exports.Reporter(settings);

  return client.setReporter(reporter).initialize();
};

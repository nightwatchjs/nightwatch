...
Nightwatch.prototype.startSession = function () {
  ...
  function getSession(retries) {
    ...
    .on('error', function(data, err) {
      ...
      if (retries && retries > 0) {
        ...
        if (self.options.selenium_session_retry_delay) {
          setTimeout(function() { getSession(retries - 1); }, self.options.selenium_session_retry_delay);
        } else {
          getSession(retries - 1)
        }
  ...
  getSession(self.options.selenium_session_retry_attempts);
  return this;
};

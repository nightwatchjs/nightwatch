...
Nightwatch.prototype.setOptions = function(options) {
  ...
  if (screenshots && screenshots.enabled) {
    if (!sceenshots.path || !fs.existsSync(sceenshots.path)) {
  ...
  if (typeof this.options.logger === 'object') {
    var enable = this.options.logger && this.options.logger.enable;
    var colors = this.options.logger && this.options.logger.colors;
    if (enable) {
      Logger.enable();
      if (!colors) {
    ...
    if (callback) {
      try {
        if (typeof callback != 'function') {
          var error = new Error('Callback parameter is not a function - ' +
            typeof(callback) + ' passed: "' + callback + '"');

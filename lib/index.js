...
const {NightwatchConfigurator} = require('@nightwatch/setup-tools');
...
Nightwatch.cli = function(callback) {
  ...
  } else if (NightwatchConfigurator.hasSupportedFlags(argv)) {
    const nightwatchConfigurator = new NightwatchConfigurator(argv);

    return nightwatchConfigurator.run();
  } else if (argv.help) {
    ArgvSetup.showHelp();
  } else if (argv.info) {
    // eslint-disable-next-line no-console

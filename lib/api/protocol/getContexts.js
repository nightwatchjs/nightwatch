 ...
 * @example
 * module.exports = {
 *   'get available contexts': function (client) {
 *     client.getContexts(function (result) {
 *       console.log('the available contexts are:', result.value);
 *     });
 *   },
 *
 *   'get available contexts with ES6 async/await': async function (client) {
 *     const contexts = await client.getContexts();
 *     console.log('the available contexts are:', contexts);
 *   }
 * };
 *
 * @syntax .getContexts([callback])
 * @method getContexts

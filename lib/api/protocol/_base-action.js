...
module.exports = class ProtocolAction {
  ...
  async locate({id, element, parentElement, commandName}) {
    ...
    const commandToReturnSingleElementMap = {
      element: true,
      elements: false,
      elementIdElement: true,
      elementIdElements: false,
    };

    const transportAction = commandToActionMap[commandName];
    const returnSingleElement = commandToReturnSingleElementMap[commandName];
    ...
    if (parentElement) {
      let elementResponse = await this.elementLocator.findElement({element: parentElement, transportAction: 'locateMultipleElements', returnSingleElement: true});
    ...
    return this.elementLocator.executeProtocolAction({id, element, transportAction, returnSingleElement, commandName});
  }

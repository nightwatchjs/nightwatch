 ...
 * If function passed except last argument, all arguments are the element selector, either specified as a string or as an object (with 'selector' and 'locateStrategy' properties).
 *
 * @example
 * module.exports = {
 *  'demo Test': function(browser) {
 *     const resultElement = await browser.findElementMultipleORCriteria('.features-container', 'li:first-child', '//p');
 *
 *     console.log('Element Id:', resultElement.getId());
 *   },
 *
 *
 * @link /#find-element-multiple-or-criteria
 * @syntax browser.findElementMultipleORCriteria(selector-1, selector-2, selector-n, callback)
 * @syntax await browser.findElementMultipleORCriteria(selector-1, selector-2, selector-n, callback);
 * @param {string, object} selector The search target.

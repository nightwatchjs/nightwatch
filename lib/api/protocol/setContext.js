 ...
 * @example
 * module.exports = {
 *   'switch to webview context': async function (client) {
 *     client
 ...
 *         const contexts = await this.getContexts();  // contexts: ['NATIVE_APP', 'WEBVIEW_<id>']
 *         await this.setContext(contexts[1]);
 *       });
 *   },
 *
 *   'switch to native context': function (client) {
 *     client.setContext('NATIVE_APP');
 *   }
 * };
 *
 * @syntax .setContext(context, [callback])
 * @method setContext

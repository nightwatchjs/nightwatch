...
/**
 * Sets the context to be automated. Used when testing hybrid mobile apps using Appium.
 *
 * More info here: https://appium.io/docs/en/commands/context/set-context/
 *
 * @example
 * module.exports = {
 *   'switch to webview context': function (browser) {
 *     browser
 ...
 *         const contexts = await this.getContexts();  // contexts: ['NATIVE_APP', 'WEBVIEW_<package name>']
 *         await this.setContext(contexts[1]);
 *       });
 *   },
 *
 *   'switch to native context': function (browser) {
 *     browser.setContext('NATIVE_APP');
 *   }
 * };
 *

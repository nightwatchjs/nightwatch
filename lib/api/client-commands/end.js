...
End.prototype.command = function(callback) {
  ...
  if (client.sessionId) {
    if (this.testFailuresExist() && this.shouldTakeScreenshot()) {
	  ...
	  if (this.shouldTakeParentScreenshot()) {
	    client.api.saveParentScreenshot(fileNamePath, function(result, err) {
	  	if (err || result.status !== 0)  {
	  	  Logger.warn('Error saving parent screenshot...', err || result);
	  	}
	    });
	  } else {
...
End.prototype.shouldTakeParentScreenshot = function() {
  return this.client.options.screenshots.capture_parent;
};

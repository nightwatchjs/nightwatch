...
class SaveScreenshot extends ClientCommand {
  performAction(callback) {
    ...
    this.api.screenshot(this.api.options.log_screenshot_data, async function(result) {
      if (client.transport.isResultSuccess(result) && result.value) {
        const file = await Screenshots.writeScreenshotToFile(fileName, result.value);
        if (file) {

...
class SaveScreenshot extends ClientCommand {
  performAction(callback) {
    ...
    console.log('Binayak', 'SaveScreenshot called');

    this.api.screenshot(this.api.options.log_screenshot_data, function(result) {
      console.log('Binayak', 'SaveScreenshot screenshot taken');

      if (client.transport.isResultSuccess(result) && result.value) {
        console.log('Binayak', 'SaveScreenshot writeScreenshotToFile called', {result});

        Screenshots.writeScreenshotToFile(fileName, result.value, err => {

          console.log('Binayak', 'SaveScreenshot writeScreenshotToFile complete', {err});
          if (err) {

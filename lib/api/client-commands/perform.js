 ...
 * @param {string} commandName The command name from which this command is fired. Defaults to undefined
...
class Perform extends EventEmitter {
  command(callback = function() {}, commandName = undefined) {
    let doneCallback;
    let asyncHookTimeout = this.client.settings.globals.asyncHookTimeout;

    this.timeoutId = setTimeout(() => {
      const extendedErrorMessage = commandName ? ` This perform was fired from ${commandName}` : '';
      this.emit('error', new Error(`Timeout while waiting (${asyncHookTimeout}ms) for the .perform() command callback to be called.` + extendedErrorMessage));
    }, asyncHookTimeout);

...
class Frame extends ClientCommand {
  async command(frameId, callback) {
    ...
    //Check if frameId is an HTML element in DOM and convert it into its CSS selector representation as frameId can only be of type number,null or string.

    if (frameId instanceof Element) {
      frameId = frameId.selector;
    }

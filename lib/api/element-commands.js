...
module.exports = function(client) {
  ...
  function CommandAction(using, value, protocolAction, args, callback) {
    ...
    var $this = this;
    var protocolCommand = getProtocolCommand(using);
    var el = protocolCommand(using, value, function(result) {
      if (result.status !== 0) {
  ...
  function getProtocolCommand(using) {
    if (using == 'recursion') {
   ...
   * @api protocol
   */
  function elementByRecursion(using, elements, callback) {
    ...
    var checkResult = function(result) {
      if (result.status !== 0) {
        ...
        if (nextElement) {
          Protocol.elementIdElement(parentId, nextElement.locateStrategy, nextElement.selector, checkResult);
        } else {
          callback(result);
        }
      }
    ...
    Protocol.element(topElement.locateStrategy, topElement.selector, checkResult);
  }

...
module.exports = function(client) {
  return {
    ...
    /**
     * Take a screenshot of the parent element (if in an iframe, current page if not in an iframe) and saves it as the given filename.
     *
     * ```
     *  this.demoTest = function (browser) {
     *    browser.saveParentScreenshot('/path/to/fileName.png');
     *  };
     * ```
     *
     * @method saveParentScreenshot
     * @param {string} fileName The complete path to the file name where the screenshot should be saved.
     * @param {function} [callback] Optional callback function to be called when the command finishes.
     * @see screenshot
     * @api commands
     */
    saveParentScreenshot : function(fileName, callback) {
      var self = this;

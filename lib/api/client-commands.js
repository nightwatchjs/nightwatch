  ...
  let Commands = {
    ...
    saveScreenshot(fileName, callback = noopFn) {
      return this.screenshot(client.options.log_screenshot_data, result => {
        Utils.writeScreenshotToFile(fileName, result.value, err => {
  ...
  return new Proxy(Commands, {
    get(target, name) {
      return function(...args) {
        ...
        let method = target[name].shift();
        let combinedArgs = target[name].concat(args);

        return this[method](...combinedArgs);
      };
    }
  });
};

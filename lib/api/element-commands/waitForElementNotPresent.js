...
class WaitForElementNotPresent extends WaitForElement {
  constructor(opts) {
    ...
    this.setNeedsRetry(false);
    this.expectedValue = 'not found';
  }

  elementFound(response) {
    this.setNeedsRetry(true);

    if (this.shouldRetryLocate()) {
      // element was found when it shouldn't be, schedule another check
  ...
  elementNotFound(response) {
    ...
    return this.pass(response, defaultMsg, response.now - this.startTimer);
  }
}

      ...
      .queueAction({
        ...
        errorHandler: (err) => {
          ...
          if (response && response.error instanceof Error) {
            return this.fail(response, `${response.error.name} - ${(response.error.message.replace(/\n/g, ''))}`, this.expectedValue, 'Timed out while waiting for element <%s> for %d milliseconds');
          }

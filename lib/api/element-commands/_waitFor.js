...
class WaitForElement extends ElementCommand {
  ...
  /**
   * if the first two arguments are strings, assume that the first one is the
   * locate strategy and remove it from the args array
   *
   * waitForElement('css selector', 'body', function() {});
   */
  setStrategyFromArgs() {
    ...
    return NightwatchAssertion.create(...args)
      ...
      .then(err => {
        if (err instanceof Error) {
          ...
          return this.complete(err, response);
        }
  ...
  formatMessage(defaultMsg, timeMs) {
    return Utils.format(this.message || defaultMsg, this.element.selector, timeMs || this.timeoutMs);
  }
}

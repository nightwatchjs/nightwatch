...
class WaitForElement extends ElementCommand {
   ...
   *  waitForElement('body');
   *
    - with a specified locate strategy
   *  waitForElement('css selector', 'body');
   *
  ...
  setArguments(opts) {
    ...
    // if the first two arguments are strings, assume that the first one is the locate strategy
    // and remove it from the args array
    if (Utils.isString(this.args[0]) && Utils.isString(this.args[1])) {
      this.__strategy = this.args.shift();
    }

    this.__selector = this.args[0];

    if (Utils.isFunction(this.args[1])) {
      ////////////////////////////////////////////////
    ...
    // backwards compatibility
    else if (Utils.isBoolean(this.args[2])) {
      ////////////////////////////////////////////////
      ...
      callback = Utils.isFunction(this.args[2]) ? this.args[2] : function() {};
    }
    ...
    // set a custom message (DEPRECATED)
    this.message = null;
    if (this.args.length > 2) {
      const lastArgument = this.args[this.args.length - 1];
      if (Utils.isString(lastArgument)) {
  ...
  executeProtocolAction(actionName) {
    return this.transport.executeProtocolAction(actionName, [this.elementId]);
  }

...
class WaitForElement extends EventEmitter {
  ...
  // FIXME: refactor this together with element-commands.js
   ...
   */
  locate() {
    return this.elementLocator.locate(this.element, this.protocolCommand)
      .then(response => {
        if (response && response.value) {
          this.elementId = response.value;

          return this.elementFound(this.elementId, response.now);
        }

        return this.elementNotFound(response.result, response.now);
      }).catch(err => {
        this.emit('error', err);
      });
  }

 ...
 * @override
 */
WaitForElement.prototype.elementFocused = function (result, now) { };

/*!
 * @override
 */
WaitForElement.prototype.elementNotFocused = function (result, now) { };
...
/*!
 * Will start checking if the element is focused and if not re-schedule the check
 * until the timeout expires or the condition has been met
 */
WaitForElement.prototype.isFocused = function () {
  var self = this;
  this.protocol.elementActive(function (result) {
    var now = new Date().getTime();
    if (result.status === 0 && result.value && result.value.ELEMENT === self.element) {
      // element was focused
      return self.elementFocused(result, now);
    }

    return self.elementNotFocused(result, now);
  });
};

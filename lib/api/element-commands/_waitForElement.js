...
class WaitForElement extends EventEmitter {
  ...
  pass(result, defaultMsg, timeMs) {
    ...
    this.assert(result, true, {}, null, this.message, this.abortOnFailure);
  ...
  assert(result, ...args) {
    return NightwatchAssertion.create(...args).run(this.client.reporter, this);
  }
 ...
 */
  locateElement() {
    return this.getProtocolCommand().then(elementIds => {
      if (elementIds instanceof Error) {
        throw elementIds;
      }
    ...
    return this.transport.locateMultipleElements(this.element)
      .catch(result => {
        if (this.transport.invalidSessionError(result)) {
          return new Error(`An error occured while running ".${this.commandName}()" command: ${this.transport.getErrorMessage(result)}.`);
        }

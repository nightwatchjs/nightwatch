...
class ClickElement extends BaseElementCommand {
  ...
  getWebElement(webElementOrString) {
    ...
    if (isString(webElementOrString)) {
      return new WebElement(this.transport.driver, webElementOrString);
    }

    throw new Error(`Unknown element: ${webElementOrString}`);
  }

  async executeProtocolAction(actionName, args = []) {
    ...
    if (this.transport.api.isSafari() && this.transport.api.isIOS()) {
      elementId = this.getWebElement(elementId);
      return this.transport.executeProtocolAction({actionName: 'executeScript', args: ['arguments[0].click();', elementId, ...args], sessionId});
    }

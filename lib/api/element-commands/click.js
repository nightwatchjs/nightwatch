 ...
 *
 * Starting with v1.1 `click()` will also wait for the element to be present (until the specified timeout). * If the element is not found, an error is thrown which will cause the test to fail. Starting with `v1.2` you can suppress element not found errors by specifying the `supressNotFoundErrors` flag.
 *
 * @example
 * module.exports = {
 *   demoTest(browser) {

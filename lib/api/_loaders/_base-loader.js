...
class BaseLoader extends EventEmitter {
  ...
  define(parent = null) {
    ...
    // do not load these namespaces on page objects and within-context.
    const namespacesRestrictedForParent = ['client'];

    const restrictedNamespace = this.namespace
      && namespacesRestrictedForParent.some((ns) => this.namespace.toString().startsWith(ns));
    if (!parent || !restrictedNamespace) {
    ...
    // do not load commands again as named exports for parent (page-objects or within-context)
    if (this.namespace && this.namespace.length && !parent) {
      this.validateMethod(parent, namespacedApi);
    ...
    nsAliases.forEach((ns) => {
      if (parent && namespacesRestrictedForParent.includes(ns[0])) {
        return;
      }
      ...
      if (ns.length && !parent) {
        // commands are being put on a namespace
        this.validateMethod(parent, namespacedApi);
        const args = this.defineArgs(parent, namespacedApi);
    ...
    return function queuedCommandFn(...args) {
      ...
      if (isES6Async || options.alwaysAsync || node.isES6Async) {
        ...
        if (parent && parent.__pageObjectItem__) {
          ...
          Object.assign(node.deferred.promise, apiToReturn);
        } else {

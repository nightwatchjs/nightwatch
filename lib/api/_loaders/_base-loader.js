...
class BaseLoader extends EventEmitter {
  ...
  static createDriverCommand(transport, commandName) {
  ...
  createQueuedCommandFn({parent, commandName, namespace, commandFn, context}) {
    ...
    return function queuedCommandFn(...args) {
      ...
      let isES6Async = options.alwaysAsync;
      if (!isES6Async) {
        isES6Async = Utils.isUndefined(this.isES6Async) ? nightwatchInstance.isES6AsyncTestcase : isAsyncCommand;
      }
      ...
      if (isES6Async || options.alwaysAsync) {
  ...
  loadDriverCommands({commands, namespace}) {
    commands.forEach(propertyName => {
      const commandFn = BaseLoader.createDriverCommand(this.transport, propertyName);

...
const ScopedElementAPILoader = require('./scoped-element-api-loader.js');
...
class BaseLoader extends EventEmitter {
  ...
  defineArgs(parent) {
    const { commandName } = this;
		...
		let commandFn = this.commandFn.bind(this);
    ...
    if (
      commandName === 'element' &&
      !this.nightwatchInstance.settings.backwards_compatibility_mode
    ) {
      args[0] = new ScopedElementAPILoader(

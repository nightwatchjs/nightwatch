      ...
      this.commandFn = function commandFn({args, stackTrace, options}) {
        return this.resolveElementSelector(args)
          .then(elementResult => {
            ...
            const customTimeout = (Utils.isObject(args[0]) && !Utils.isUndefined(args[0]['timeout'])) ? args[0]['timeout'] : (Utils.isNumber(instance.retryAssertionTimeout) ? instance.retryAssertionTimeout : this.timeout)

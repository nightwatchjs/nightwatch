...
class BaseCommandLoader extends BaseLoader {
  ...
  validateMethod(parent, namespacedApi) {
    let namespace = this.getTargetNamespace(parent, namespacedApi);
    if (Array.isArray(namespace) && namespace.length > 0) {
      namespace = BaseLoader.unflattenNamespace(namespacedApi || this.api, namespace.slice());
    }
    ...
    if (this.nightwatchInstance.isApiMethodDefined(this.commandName, namespace, namespacedApi) && !this.nightwatchInstance.overridableCommands.has(this.commandName)) {

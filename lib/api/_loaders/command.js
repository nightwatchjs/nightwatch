      ...
      this.commandFn = function commandFn({args, stackTrace}) {
        ...
        const result = this.resolveElementSelector(args)
          .then(elementResult => {
            ...
            if (Utils.isObject(instance.client.settings.globals.commandTimeout)) {
              setTimeout(() => {
                if (instance instanceof EventEmitter) {
                  instance.emit('error', new Error(`Command Timeout - Exceeded ${instance.client.settings.globals.commandTimeout} ms`));
                }
              }, instance.client.settings.globals.commandTimeout);
            }

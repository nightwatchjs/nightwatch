...
class CommandLoader extends BaseCommandLoader {
  ...
  static createFromObject(CommandModule) {
    return class CommandClass extends EventEmitter {
      command(...args) {
        console.log("hulla", CommandModule);
      ...
      this.commandFn = function commandFn({args, stackTrace}) {
        ...
        const result = this.resolveElementSelector(args)
          .then(elementResult => {
            ...
            setTimeout(()=> instance.emit('error', new Error(`Command Timeout - Exceeded ${instance.client.settings.globals.commandTimeout} ms`)), instance.client.settings.globals.commandTimeout);

    ...
    class CommandInstance extends CommandClass {
      reportProtocolErrors(result) {
        if (opts.isUserDefined) {
          ...
          return super.registerTestError(result);
        }
      }
          ...
          .then(result => {
            ...
            if (result && result.status === -1 && instance.reportProtocolErrors(result) && reportErrors) {
              ...
              if (result.error instanceof Error) {
                err = result.error;
                err.message = `Error while running .${this.commandName}(): ${result.error.message}`;
              } else {
                err = new Error(`Error while running .${this.commandName}(): ${result.error}`);

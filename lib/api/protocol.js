...
module.exports = class ProtocolActions {
  ...
  get Actions() {
    return {
       ...
       * @param {string} webElementId The web element ID of the element to route the command to.
       ...
       *
       * Please note that this element operates on a protocol level and requires a [Web Element ID](https://www.w3.org/TR/webdriver1/#dfn-web-elements). Read more on [Element retrieval](https://www.w3.org/TR/webdriver1/#element-retrieval) on the W3C WebDriver spec page.
       *
       * @example
       * module.exports = {
       *  'demo Test' : function(browser) {
       ...
       *
       * @link
       * @param {string} webElementId The web element ID assigned to the element to move to. If not specified or is null, the offset is relative to current position of the mouse.
   ...
   * @return {Promise}
   */
  findElements({element, id, using, value, commandName, callback = function() {}}) {
    ...
    ProtocolActions.validateStrategy(element.locateStrategy, commandName);

    return this.locate({id, element, commandName})
      .then(result => {

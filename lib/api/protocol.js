...
module.exports = function(nightwatchInstance) {
  ...
  Actions.session = function(action, sessionId, callback) {
    ...
    return TransportActions.sessionAction(action.toUpperCase(), sessionId, callback);
  };
  ...
  function isValidStrategy(using) {
    ...
    if (!isValid) {
      ...
      throw new Error(`Provided locating strategy "${using}" is not supported. It must be one of the following: ${list}`);
    }
   ...
   */
  Actions.screenshot = function(log_screenshot_data = false, callback) {
  ...
  Actions.setOrientation = function (orientation, callback) {
    ...
    if (!ScreenOrientation.indexOf(orientation) > -1) {

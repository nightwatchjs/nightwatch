...
module.exports = function(nightwatchInstance) {
  ...
  Actions.session = function(action, sessionId, callback) {
    ...
    return TransportActions.session(action.toUpperCase(), sessionId, callback);
  };
  ...
  // Element related
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  function isValidStrategy(using) {
    let usingLow = String(using).toLocaleLowerCase();

    let isValid = Object.keys(LocateStrategy).some(key => {
      return usingLow === LocateStrategy[key];
    });

    if (!isValid) {
      let list = Object.keys(LocateStrategy).map(k => LocateStrategy[k]).join(', ');
      throw new Error(`Provided locating strategy is not supported: ${using}. It must be one of the following: ${list}`);
    }
   ...
   * @private
   */
  function findElement(elem, callback) {
    if (isValidStrategy(elem.locateStrategy)) {
      return TransportActions.locateSingleElement(elem.locateStrategy, elem.selector, callback);
    }
  }
   ...
   * @private
   */
  function findElements(elem, callback) {
    if (isValidStrategy(elem.locateStrategy)) {
      return TransportActions.locateMultipleElements(elem.locateStrategy, elem.selector, callback);
    }
  }
  ...
  function filterElementsForCallback(elem, callback) {
    ...
    return callback && Transport.filterElementsForCallback(elem, callback);
  }
  ...
  Actions.elements = function(using, value, callback) {
    ...
    return findElements(elem, callback);
  };
  ...
  Actions.elementIdElements = function(id, using, value, callback) {
    ...
    if (isValidStrategy(elem.locateStrategy)) {
      return TransportActions.locateSingleElementByElementId(id, elem.locateStrategy, elem.selector,
  ...
  Actions.setOrientation = function (orientation, callback) {
    ...
    if (!ScreenOrientation.contains(orientation)) {

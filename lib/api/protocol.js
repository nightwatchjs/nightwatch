...
module.exports = function(Nightwatch) {
  ...
  Actions.element = function(using, value, callback) {
    ...
    if (elem.locateStrategy == 'recursion') {
      return new elementByRecursion(Nightwatch).command(elem.selector, callback); // eslint-disable-line new-cap
    }
  ...
  Actions.elements = function(using, value, callback) {
    var elem = Element.fromSelector(value, using);
    if (elem.locateStrategy == 'recursion') {
      return new elementsByRecursion(Nightwatch).command(elem.selector, callback); // eslint-disable-line new-cap
    }

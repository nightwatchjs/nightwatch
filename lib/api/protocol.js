...
module.exports = function(Nightwatch) {
  ...
  Actions.element = function(using, value, callback) {
    var strategies = ['class name', 'css selector', 'id', 'name', 'link text',
      'partial link text', 'tag name', 'xpath', 'recursion'];
    ...
    if (using === 'recursion') {
      return this.elementByRecursion(value, callback);
    }
   ...
   * @api protocol
   */
  Actions.elementByRecursion = function(elements, callback) {
    ...
    var checkResult = function(result) {
      if (result.status !== 0) {
        ...
        if (nextElement) {
          this.elementIdElement(parentId, nextElement.locateStrategy, nextElement.selector, checkResult);
        } else {
          callback(result);
        }
      }
    ...
    this.element(topElement.locateStrategy, topElement.selector, checkResult);
  };
  ...
  Actions.elements = function(using, value, callback) {
    var check = /class name|css selector|id|name|link text|partial link text|tag name|xpath|recursion/gi;
    if (!check.test(using)) {
      throw new Error('Please provide any of the following using strings as the first parameter: ' +
        'class name, css selector, id, name, link text, partial link text, tag name, xpath, or recursion');
    }

    if (using === 'recursion') {
      return this.elementsByRecursion(value, callback);
    }
   ...
   * @api protocol
   */
  Actions.elementsByRecursion = function(elements, callback) {
    var allElements = elements.slice();
    var self = this;

    var deferredElementIdElements = function(el, using, value) {
      var deferred = Q.defer();
      self.elementIdElements(el, using, value, function(result) {
        deferred.resolve(result);
      });
      return deferred.promise;
    };

    var createResult = function(result, foundElements) {
      return {
        status: 0,
        sessionId: result.sessionId,
        value: foundElements.map(function(e) { return { ELEMENT: e };}),
        class: result.class,
        hCode: result.hCode
      };
    };
    ...
    var topElement = allElements.shift();
    this.elements(topElement.locateStrategy, topElement.selector, checkResult);
  };

...
module.exports = function(Nightwatch) {
   ...
   * ```
   *
   * Under the hood, any function you pass to the `execute` function is sent to the browser
   * as a string so it can be executed within the browser scope. If the `body` param is a function,
   * this is converted to a string with `<function>.toString()`.
   * Any references to your current scope are ignored.
   *
   * To ensure cross-browser compatibility, *your function
   * should not be in ES6 format (ie `() => {}`)* and should be capable of being executed with
   * `eval(<your-function-converted-to-string>)`.
   *
   * If the execution of your function fails, the first argument of the callback function

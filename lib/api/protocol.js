...
module.exports = class ProtocolActions {
  ...
  static validateStrategy(strategy, apiMethod) {
    if (!LocateStrategy.isValid(strategy)) {
      const list = LocateStrategy.getList();

      throw new Error(`Provided locating strategy "${strategy}" is not supported for .${apiMethod}{}. It must be one of the following: ${list}.`);
    }
  }
  ...
  get Actions() {
    return {
      ...
      element(using, value, callback) {
        return this.findElements(using, value, 'element', callback);
      },
      ...
      elements(using, value, callback) {
        return this.findElements(using, value, 'elements', callback);
      },
      ...
      elementIdElement(id, using, value, callback) {
        ...
        ProtocolActions.validateStrategy(using, 'elementIdElement');

        return this.TransportActions.locateSingleElementByElementId(id, using, value, callback);
      },
      ...
      elementIdElements(id, using, value, callback = function() {}) {
        ...
        ProtocolActions.validateStrategy(using, 'elementIdElements');

        return this.TransportActions.locateMultipleElementsByElementId(id, using, value, callback);
      },
   ...
   * @return {Promise}
   */
  findElements(using, value, commandName, callback = function() {}) {
    ProtocolActions.validateStrategy(using, commandName);
    const elem = Element.createFromSelector(value, using);

    return this.locate(elem, commandName)
      .catch(err => {
        const now = new Date().getTime();
        const errorMessage = err.message ? err.message || `An unknown error occurred while running .${commandName}()`;
        ...
        callback.call(this.nightwatchInstance.api, response.result);
   ...
   */
  locate(element, protocolCommand) {
    let action;

    switch (protocolCommand) {
      case 'element':
        action = 'locateSingleElement';
        break;
      case 'elements':
        action = 'locateMultipleElements';
        break;
    }

    return this.elementLocator.executeProtocolAction(element, action, protocolCommand);
  }

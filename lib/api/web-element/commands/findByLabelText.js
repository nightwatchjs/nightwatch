 ...
 * @instance
 * @syntax browser.element.findByLabelText(text, tagName, [options])
 * @param {string} text
 * @param {string} tagName
 * @param {{exact: boolean}} [options]
 * @returns {ScopedWebElement}
 */
module.exports.command = function (text, tagName, {exact = true, timeout, retryInterval, suppressNotFoundErrors} = {}) {

  const findByForId = async (instance, labelElement, tagName) => {
    if (!labelElement) {
      return null;
    }
    ...
    const element = await instance.waitUntilElementsLocated({
      selector: By.css(`${tagName}[id="${forAttribute}]`),
      timeout,
      retryInterval
    });
  ...
  const findByAriaLabelled = async (instance, labelWebElement, tagName) => {
    if (!labelWebElement) {
      return null;
    }
    ...
    return instance.waitUntilElementsLocated({
      selector: By.css(`${tagName}[aria-labelledby="${idAttribute}"]`),
      timeout,
      retryInterval
    });
  };

  const findByDirectNesting = async (labelWebElement, tagName) => {
    if (!labelWebElement) {
      return null;
    }

    try {
      return await labelWebElement.findElement(By.css(tagName));
    } catch (err) {
      return null;
    }
  };

  const findByDeepNesting = async (text, {exact}, tagName) => {
    ...
    try {
      return await labelElement.findElement(By.css(tagName));
    } catch (err) {
      return null;
    }
  };

  const findByAriaLabel = async (text, {exact}, tagName) => {
    const labelElement = await this.find({
      selector: By.css(`${tagName}[aria-label${exact ? '' : '*'}="${text}"]`),
      timeout,
      retryInterval,
      suppressNotFoundErrors: true
    });
  ...
  const findFromLabel = async (instance, labelElement, tagName) => {
    let element = null;

    if (labelElement) {
      try {
        element = await findByForId(instance, labelElement, tagName);
      } catch (err) {
        // ignore
      }
      ...
      if (!element) {
        try {
          element = await findByAriaLabelled(instance, labelElement, tagName);
        } catch (err) {
          // ignore
        }
      }
      ...
      if (!element) {
        try {
          element = await findByDirectNesting(labelElement, tagName);
        } catch (err) {
          // ignore
        }
      }
  ...
  const createAction = function (labelElement, tagName) {
    const instance = this;

    return async function() {
      const element = await findFromLabel(instance, labelElement, tagName);
  ...
  return this.createScopedElement(new Promise(async (resolve, reject) => {
    ...
    if (labelElement) {
      const node = this.queueAction({name: 'findByLabelText', createAction: function () {
        return createAction.call(this, labelElement, tagName);
      }});
    ...
    const byDeepNesting = await findByDeepNesting(text, {exact}, tagName);
    if (byDeepNesting) {
      return resolve(byDeepNesting);
    }

    const byAriaLabel = await findByAriaLabel(text, {exact}, tagName);
    if (byAriaLabel) {

...
class ScopedWebElement {
  ...
  async findElementAction({parentElement, condition, index, timeout, retryInterval}) {
  ...
  createWebElementPromise({selector, parentElement}) {
    ...
    return new WebElementPromise(this.driver, new Promise(async (resolve, reject) => {
      ...
      try {
        const webElement = await this.findElementAction({parentElement, condition, index, timeout, retryInterval});

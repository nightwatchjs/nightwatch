      ...
      value(message) {
        return instance.assert(async (assertApi, element) => {
          try {
            ...
            const isNegated = instance.negated;
            if (!el && !isNegated) {
              return assertApi.fail(message || 'Expected element to be present, but it is not found.');
            } else if (el && isNegated) {
              return assertApi.fail(message || 'Expected element to be absent, but it is present.');
            } else if (el && !isNegated) {
              return assertApi.ok(true, message || 'Testing if the element is absent.');
            }
            ...
            if (instance.negated && error.name === 'NoSuchElementError') {
              return assertApi.ok(true, message || 'Testing if the element is absent.');
            }
            throw error;
          }

...
class XPositionAssertion extends BaseAssertion {
  ...
  init() {
    ...
    this.flag('xPositionFlag', true);
    this.message = 'Expected element <%s> xPosition to' + (this.negate ? ' not' : '');
    this.start();
  }
  ...
  onPromiseResolved(elementId) {
    ...
    this.executeCommand().then(result => {
      ...
      this.resultValue = result.value.x;
...
module.exports = XPositionAssertion;

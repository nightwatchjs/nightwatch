...
class ApiLoader {
  ...
  loadPlugins(parent = null) {
    ...
    plugins.forEach(pluginName => {
      ...
      if (plugin.transforms) {
        this.__loadPluginTransforms(plugin.transforms);
      }
    });
  }
  ...
  __normalizeTransforms(transforms) {
    return Promise.resolve((typeof transforms === 'function' ? transforms() : transforms) || []);
  }
  ...
  __loadPluginTransforms(nextTransforms) {
    this.nightwatchInstance.transforms = this.nightwatchInstance.transforms
      .then(existingTransforms => {
        return this.__normalizeTransforms(nextTransforms).then(items => {
          return existingTransforms.concat(items);
        });
      });
  }
}

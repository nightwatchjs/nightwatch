...
class ApiLoader {
  ...
  get CommandFiles() {
    return {
      ...
      assertions: {
        ...
        namespaces: {
          assert: {
            abortOnFailure: this.api.globals.abortOnAssertionFailure
          },
          verify: {
  ...
  loadPlugins(parent = null) {
    ...
    plugins.forEach(pluginName => {
      const instance = new PluginLoader(this.nightwatchInstance);
      const plugin = instance.load(pluginName);
      ...
      if (plugin.assertions) {
        this.__loadCustomObjectsSync({
          ...
          namespaces: this.CommandFiles.assertions.namespaces,
          Loader: AssertionLoader
        });
      }
    });
  ...
  loadApiCommandsSync(parent = null) {
    Object.keys(this.CommandFiles)
      .forEach((key) => {
        const commands = this.CommandFiles[key];

  ...
  __loadCommandsSync({
    ...
    if (namespacesObj) {
      ...
      Utils.readFolderRecursively(dirPath, [], (dirPath, fileName, namespace) => {
        ...
        const parsedResource = path.parse(fileName);
        const jsFileExists = visited[path.join(dirPath, parsedResource.name + Utils.jsFileExt)];
        if (isUserDefined && parsedResource.ext === Utils.tsFileExt && jsFileExists) {
          return;
        }
        visited[path.join(dirPath, fileName)] = true;

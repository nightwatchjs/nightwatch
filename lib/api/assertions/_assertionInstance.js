...
class AssertionInstance {
  constructor({nightwatchInstance, args, fileName, options}) {
    ...
    this.__retryAssertionTimeout = Array.isArray(args) && args[0] && args[0].timeout;
    this.__rescheduleInterval = Array.isArray(args) && args[0] && args[0].retryInterval
  ...
  get rescheduleInterval() {
    return this.__rescheduleInterval;
  }

  set rescheduleInterval(value){
    this.__rescheduleInterval = value;
  }

  get retryAssertionTimeout() {
    return this.__retryAssertionTimeout;
  }

  set retryAssertionTimeout(value) {
    this.__retryAssertionTimeout = value
  }
  ...
  getValue() {
    ...
    if (!this.result || this.result.value === undefined) {
      return null;
    }

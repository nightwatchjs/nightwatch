...
const Transport = require('../selenium.js');
...
class TransportActions {
  ...
  static get methodMappings() {
    return {
      ...
      setWindowSize(windowHandle, offsetX, offsetY) {
        return TransportActions.post({
  ...
  static sendRequest(options) {
    if (typeof options == 'string') {
      options = TransportActions.createOptions(options);
    }

    return Transport.runProtocolAction(options);
  }
}

module.exports = function() {
  return new Proxy(new TransportActions(), {
    get(target, name) {
      return function(...args) {

        let mapping = TransportActions.methodMappings[name];
        if (typeof mapping == 'function') {
          return TransportActions.sendRequest(mapping(...args));
        }

        return TransportActions.sendRequest(mapping);
      }
    }
  });
};

...
class CustomRunnable {
  ...
  static run(fn, isHook = false) {
    ...
    let emitted;
    if (!isHook) {
      this.parent.client.isES6AsyncTestcase = Utils.isES6AsyncFn(ctx.test.fn);
...
class MochaRunner {
    ...
    class CustomRunner extends Mocha.Runner {
      runSuite(suite, fn) {
        ...
        let createSession = new Promise(function(resolve, reject) {
          if (isMainSuite) {
            try {
              const {settings} = suite.client;
              const modulePath = suite.file;
              const argv = require('../cli/argv-setup.js').argv;
              const modules = [];
              const testSuite = new TestSuite({modulePath, modules, settings, argv});
              const context = new Context({modulePath, settings, argv});

              context.init();
              suite.nightwatchSuite = testSuite;

              testSuite.createClient(suite.client);
              testSuite.createContext({context});

              testSuite.createSession()
  ...
  constructor(settings, argv, addtOpts) {
    ...
    const MochaNightwatch = MochaRunner.MochaNightwatch();

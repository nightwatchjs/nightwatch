...
class DefaultRunner {
  ...
  closeOpenSessions(err) {
    if (this.currentSuite && this.currentSuite.inspectorServer) {
      Logger.info(`Attempting to close websocket server running on port ${this.currentSuite.inspectorServer.portNumber}...`);
      
      this.currentSuite.inspectorServer.closeSocket();
    }

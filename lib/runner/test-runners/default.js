...
class DefaultRunner {
  ...
  closeOpenSessions(err) {
    if (this.playgroundServer) {
      Logger.info(`Attempting to close websocket server running on port ${this.playgroundServer.portNumber}...`);
      
      this.playgroundServer.closeSocket();
    }
  ...
  async createTestSuite({modulePath, modules}) {
    const {settings, argv, addtOpts, playgroundServer} = this;

    const testSuite = new TestSuite({modulePath, modules, settings, argv, addtOpts, playgroundServer});

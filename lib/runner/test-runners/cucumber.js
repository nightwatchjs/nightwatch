...
class CucumberSuite extends TestSuite {
  ...
  getCliArgvForCucumber() {
    ...
    const additionalOptions = this.buildArgvValue(['tags', 'retry-tag-filter', 'profile', 'format', 'format-options', 'dry-run', 'fail-fast', ['retry', 'retries'], 'no-strict', 'name'], options);
  ...
  buildArgvValue(argNames, options={}) {

...
class CucumberSuite extends TestSuite {
  ...
  static validateCucumber(settings) {
    let cucumberModule;
    try {
      cucumberModule = require('@cucumber/cucumber');
    } catch (err) {
  ...
  createCucumberApi() {
    this.cucumberApi = require('@cucumber/cucumber/api');
    ...
    this.cucumberOptions = this.getApiOptionsForCucumber();
  }

  setupCucumberIntegration() {
    const cucumberVersion = CucumberSuite.validateCucumber(this.settings);
    this.integrationStrategy = this.settings.test_runner.options.integrationStrategy;
    if (!this.integrationStrategy) {
      // API behavior we rely on introduced in cucumber-js 10.3.1
      this.integrationStrategy = semver.satisfies(cucumberVersion, '>=10.3.1') ? 'API' : 'CLI';
    }

    if (this.integrationStrategy === 'API') {
      this.createCucumberApi();
    } else {
  ...
  getCliOptionsForCucumber() {
    ...
    return [].concat(feature_path, parallelArgs, specs, additionalOptions, extraParams);
  }
  ...
  getCliArgvForCucumber() {
    ...
    ].concat(this.getCliOptionsForCucumber());
  }

  getApiOptionsForCucumber() {
    return {
      profiles: this.mergeCliConfigValues('profile'),
      provided: this.getCliOptionsForCucumber()
    };
  }

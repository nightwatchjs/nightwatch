...
class CucumberSuite extends TestSuite {
  ...
  getCliArgvForCucumber() {
    ...
    const parallelArgs = (this.usingCucumberWorkers && this.usingCucumberWorkers>0) ? ['--parallel', this.usingCucumberWorkers]: [];
    ...
    const profileOption = this.argv['profile'] ? ['--profile', this.argv['profile']] : [];
    ...
    ].concat(feature_path, parallelArgs, specs, tagsOption, formatArg, formatOptions, extraParams, dryRun, failFast, retryOption, profileOption, noStrict, featureNameFilter, retryTagsOption);
  }

...
module.exports = class NightwatchFormatter extends Formatter {
  ...
  static setCapabilities(data) {
    ...
    console.info('test_case_started_id (while sending session details to parent):', process.env.CUCUMBER_TEST_CASE_STARTED_ID);
  ...
  onSessionCapabilities(envelope) {
    console.info('Session data received in parent for test_case_started_id:', envelope.testCaseStartedId);
  ...
  reportHandler(envelope) {
    try {
      ...
      if (cucumberEvent === 'session') {
        console.info('Session envelope received in parent:', envelope);
      }

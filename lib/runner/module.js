...
function Module(modulePath, opts, addtOpts) {
  ...
  var specifiedTestcases = addtOpts && addtOpts.testcase;
  ...
  this.keys = [];
  if (specifiedTestcases) {
    var testCases = specifiedTestcases.split(',');
    for (var i in testCases) {
      var currentTestcase = testCases[i];
      var currentTestcaseKeys = this.getKeys(currentTestcase);
      if (currentTestcase && currentTestcaseKeys.length === 0) {
        throw new Error('Error: "' + currentTestcase + '" is not a valid testcase in the current test suite.');
      }
      this.keys = this.keys.concat(currentTestcaseKeys);
    }
  } else {
    // Get all the keys.

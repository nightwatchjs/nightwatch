    ...
    self.hooks[key] = (function(item, argsCount) {
      return function() {
        return self.makePromise(function(done, deferred) {
          ...
          self.runHookMethod(item, context, argsCount, key, doneFn, deferred);
...
TestSuite.prototype.run = function() {
  ...
  if (this.module.isDisabled()) {
    ...
    this.globalBeforeEach()
      ...
      .then(function() {
        self.testResults.errors = self.client.results('errors');
...
TestSuite.prototype.runTestSuiteModule = function() {
  ...
  return this.before()
    ...
    .catch(function(err) {
      // Intentionally failing tests in after, i.e. calling: done(err);
      self.client.handleException(err);
    });
};
...
TestSuite.prototype.runHookMethod = function(fn, context, asyncArgCount, hookName, doneFn, deferred) {
  ...
  //doneFn = this.adaptDoneCallback(doneFn, hookName, deferred);

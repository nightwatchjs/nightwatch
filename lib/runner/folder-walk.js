...
class Walk {
  ...
  readTestSource() {
    let modulePathsCopy = this.fullPaths.slice(0);

    return new Promise(function readSourcePaths(resolve, reject) {
      let sourcePath = modulePathsCopy.shift();
      ...
      this.checkPath(sourcePath)
        ...
        .then(sourcePath => {
          ...
          if (modulePathsCopy.length === 0) {
            resolve(this.results.getData());
          } else {
            readSourcePaths(resolve, reject);
          }
        })
        .catch(function(err) {
          reject(err);

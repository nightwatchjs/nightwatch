...
module.exports = new (function() {
  ...
  function getTestSuiteName(moduleName) {
    var words;
    var fileNameSpecialChars = /(_|\.)/g;

    if (fileNameSpecialChars.test(moduleName)) {
      moduleName = moduleName.replace(fileNameSpecialChars, ' ');
    } else {
      moduleName = moduleName.replace(/([A-Z])/g, ' $1');
    }
  ...
  this.run = function runner(files, opts, additional_opts, finishCallback) {
    ...
    runFiles(paths, function runTestModule(err, fullpaths) {
      ...
      if (opts.output) {
        ...
        console.log('\n' + Logger.colors.blue(testSuiteName));
        console.log(Logger.colors.blue(new Array(testSuiteName.length + 1).join('=')));
      }

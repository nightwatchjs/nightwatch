...
Runner.prototype.runTestModule = function(modulePath, fullPaths) {
  ...
  return this.currentTestSuite
    .on('testcase:finished', function(results, errors, time) {
      ...
      testSuiteResult.completed[this.currentTestSuite.currentTest] = {
    ...
    .run()
    .then(function onTestSuiteResolved(results) {
      var testSuiteResult = this.globalResults.modules[moduleKey];
      testSuiteResult.skipped = results.steps;
      testSuiteResult.errors = results.errors;
      //testSuiteResult.errmessages = results.errmessages;
      testSuiteResult.timestamp = results.timestamp;
      testSuiteResult.time = (results.time/1000).toPrecision(4);
      ...
      if (Array.isArray(results.errmessages) && results.errmessages.length) {
        this.globalResults.errmessages = this.globalResults.errmessages.concat(results.errmessages);
      }

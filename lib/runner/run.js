...
module.exports = new (function() {
  ...
  function runModule(module, opts, moduleName, moduleKey, moduleCallback, finishCallback) {
    ...
    if (typeof module.afterEach == 'function' || typeof module.tearDown == 'function') {
      ...
      index = keys.indexOf('afterEach');
      if (index == -1) {
        index = keys.indexOf('tearDown');
      }
      if (index > -1) {
        keys.splice(index, 1);
      }
      resultsIndex = testResults.steps.indexOf('afterEach');
      if (resultsIndex == -1) {
        resultsIndex = testResults.steps.indexOf('tearDown');
      }
      if (resultsIndex > -1) {
        testResults.steps.splice(resultsIndex, 1);
      }

...
CliRunner.prototype = {
  ...
  parseTestSettings : function(callback) {
    ...
    if (!this.isParallelMode() && envs.length < 2 && this.settings.testWorkers && this.settings.testWorkers.enabled ) {
      console.log('test worker mode');
      this.setupTestWorkerMode(this.argv.env, callback);
    }
  ...
  setupTestWorkerMode: function(env, callback) {
    this.parallelMode = true;
    var self = this;

    this.startSelenium(function() {
      self.startTestWorkers(env, function(o, code) {
        self.stopSelenium();
        if (callback) {
          callback();
        }
  ...
  startTestWorkers : function(nightwatchEnv, finishCallback) {
    var execFile = require('child_process').execFile, child, self = this;
    var mainModule = process.mainModule.filename;
    finishCallback = finishCallback || function() {};
        ...
        list.forEach(function(file) {
          ...
          fs.stat(file, function(err, stat) {
            if (stat && stat.isDirectory()) {
              ...
              console.log('stat file', basename);
    ...
    walk(paths[0], function(err, tests) {
      console.log('err', err, 'tests', tests);
      ...
      var queue = async.queue(function(item, cb) {
        ...
        env.__NIGHTWATCH_PARALLEL_MODE = 1;
        env.__NIGHTWATCH_ENV = nightwatchEnv;

        console.log('exec', process.execPath, cliArgs);
      ...
      queue.drain = function() {
        if (!self.processesRunning()) {
          console.log('finishCallback', arguments);
          finishCallback(output, globalExitCode);
        }
      };
    }, this.test_settings);

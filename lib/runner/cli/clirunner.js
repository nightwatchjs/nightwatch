...
CliRunner.prototype = {
  ...
  singleSourceFile: function() {
    return Array.isArray(this.argv['_']) && this.argv['_'].length == 1 && fs.statSync(this.argv['_'][0]).isFile();
  },
  ...
  getTestSource : function() {
    ...
    if (this.singleTestRun()) {
      ...
      if (Array.isArray(this.argv._) && (this.argv._.length > 0)) {
        testsource = this.argv._;
  ...
  startSelenium : function(callback) {
    ...
    if (this.settings.webdriver_only || this.isTestWorker()) {
      callback();
      return this;
    }
  ...
  stopSelenium : function() {
    ...
    if (this.settings.webdriver_only) {
      deferred.resolve();
    } else if (this.manageSelenium) {

...
CliRunner.prototype = {
  ...
  startChildProcesses : function(envs, doneCallback) {
    ...
    if (envs === null) {
      this.startTestWorkers(availColors, doneCallback);
      return this;
    }

    envs.forEach(function(environment, index) {
      self.childProcessOutput[environment] = [];
      self.runningProcesses[environment] = new ChildProcess(environment, index, self.childProcessOutput[environment], self.settings, [
        '--env', environment
      ]);

      self.runningProcesses[environment].run(availColors, function(output, exitCode) {
  ...
  startTestWorkers : function(availColors, doneCallback) {
    ...
    Runner.readPaths(source, this.test_settings, function(error, modulePaths) {
      ...
      self.processAsyncQueue(workerCount, modulePaths, function(modulePath, index, next) {
        var filename = path.basename(modulePath, '.js');

        self.childProcessOutput[filename] = [];
        self.runningProcesses[filename] = new ChildProcess(filename, index, self.childProcessOutput[filename], self.settings, [
          '--test', modulePath
        ]);

        self.runningProcesses[filename].run(availColors, function (output, exitCode) {
          var running = self.processesRunning();
          if (running > 1) {
            next();
          } else if (running === 0) {
            if (!self.settings.live_output) {
  ...
  processAsyncQueue : function(concurrency, files, cb) {

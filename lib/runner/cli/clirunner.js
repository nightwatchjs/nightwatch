...
CliRunner.prototype = {
  ...
  findGroupPath : function(groupName) {
    this.test_settings.group_name = groupName; // FIXME: this won't work when running in parallel with test workers
    ...
    if (fullGroupPath.indexOf(fullSrcFolder) === 0) {
      fullPath = groupName;
      this.test_settings.group_name = fullGroupPath.substring(fullSrcFolder.length+1);
    } else {
  ...
  runTests : function(finished) {
    ...
    this.startSelenium(function() {
      Runner.run(source, self.test_settings, {
        ...
        suite_retries : self.argv.suiteRetries

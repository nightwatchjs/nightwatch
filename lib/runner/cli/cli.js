...
class CliRunner {
  ...
  isTestWorkersEnabled() {
    return this.test_settings.testWorkersEnabled && !this.singleTestRun();
  }

  singleTestRun() {
    return Utils.isString(this.argv.test) || Array.isArray(this.argv._source) && this.argv._source.length === 1;
  }
  ...
  isWebDriverManaged() {
    if (this.test_settings.use_selenium_webdriver) {
      return false;
    }

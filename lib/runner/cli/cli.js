...
const {GlobalHookRunStarted, GlobalHookRunFinished} = DEFAULT_RUNNER_EVENTS;
...
class CliRunner {
  ...
  setupEventHub() {
    if (this.isOnEventCallbackExistsInGlobal()) {
      ...
      if (Utils.isFunction(globals.onEvent)) {
        globals.onEvent(NightwatchEventHub);
      }
  
      if (plugins.length > 0) {
        plugins.forEach((plugin) => {
          if (plugin.globals && Utils.isFunction(plugin.globals.onEvent)){
            plugin.globals.onEvent(NightwatchEventHub);
          }
        });
      }
    }
  ...
  runGlobalHook(key, args = [], isParallelHook = false) {
    ...
    if (NightwatchEventHub.isAvailable && Utils.isFunction(globals[key])) {

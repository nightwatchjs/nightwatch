...
class CliRunner {
  ...
  // Concurrency related
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////
  testWorkersMode(modules=null) {
    return this.isTestWorkersEnabled(modules) && !this.testRunner.isTestWorker();
  }

  isTestWorkersEnabled(modules=null) {
    return this.test_settings.testWorkersEnabled && !this.singleTestRun(modules);
  }

  singleTestRun(modules = null) {
    return (modules && Array.isArray(modules) && modules.length < 2) && (Utils.isString(this.argv.test) || Array.isArray(this.argv._source) && this.argv._source.length === 1);
  }

  parallelMode(modules = null) {
    if (this.testWorkersMode(modules) && Array.isArray(modules) && modules.length <= 1) {
      return false;
    }

    return this.testEnvArray.length > 1 || this.testWorkersMode(modules);
  }
  ...
  isWebDriverManaged() {
    const {webdriver} = this.test_settings;
  ...
  startWebDriver() {
    ...
    return WebDriver.startInstances().catch(err => {
      ...
      if (result.length === 1 && this.testRunner.isTestWorker()) {
        this.test_settings.webdriver.port = result[0].settings.port;
      }

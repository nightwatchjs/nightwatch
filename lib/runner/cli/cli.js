...
class CliRunner {
  ...
  createTestRunner() {
    this.test_settings.original_settings = this.settings;
  ...
  getTestsFiles() {
    return new Promise((resolve, reject) => {
      const modules = {};
      const promises = [];
      this.testEnvArray.forEach(env => {
        const envSettings = Settings.parse(this.userSettings, this.baseSettings, this.argv, env);
        const promise = Runner.readTestSource(envSettings, this.argv).then((envModules) => {
          modules[env] = envModules;
        });

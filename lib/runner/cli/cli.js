...
const {Logger, singleSourceFile, iosRealDeviceUDID, isRealIos, isSimulator} = Utils;
...
class CliRunner {
  ...
  initTestSettings(userSettings = {}, baseSettings = null, argv = null, testEnv = '') {
    ...
    if (isRealIos(this.test_settings.desiredCapabilities)) {
      if (argv.deviceId) {
        this.test_settings.desiredCapabilities['safari:deviceUDID'] = iosRealDeviceUDID(argv.deviceId);
      } else if (!this.test_settings.desiredCapabilities['safari:deviceUDID']) {
        let err = new Error('Real device id is niether configured or passed');
  ...
  get isMobile() {
    const {platformName} = this.test_settings.desiredCapabilities || {};

    return platformName ? (platformName.toLowerCase() === 'ios' || platformName.toLowerCase() === 'android') : false;
  }
  ...
  isTestWorkersEnabled() {
    ...
    if (!testWorkers) {
      ...
      Logger.info('Disabling parallelism to run tests on mobile platform');
    ...
    return this.runGlobalHook('before', [this.test_settings])
      ...
      .then(modules => {
        ...
        if (!real_mobile && avd) {
          ...
          this.androidServer = new AndroidEmulator(avd);
      ...
      .then(errorOrFailed => {
        ...
        if (this.test_settings.deviceUDID && !isRealIos(this.test_settings.desiredCapabilities)) {
          const cmd = `xcrun simctl shutdown ${this.test_settings.deviceUDID}`;

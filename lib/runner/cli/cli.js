...
const {Logger, singleSourceFile, iosRealDeviceUDID, getSdkRootFromEnv} = Utils;
...
class CliRunner {
  ...
  initTestSettings(userSettings = {}, baseSettings = null, argv = null, testEnv = '') {
    ...
    if (argv.deviceId && browserName.toLowerCase() === 'safari' && platformName.toLowerCase() === 'ios' && desiredCapabilities['safari:useSimulator'] !== true) {
  ...
  async launchMobileEmulator() {
    const {desiredCapabilities} = this.test_settings;
    const chromeOptions = desiredCapabilities['goog:chromeOptions'];

    if (chromeOptions && chromeOptions.androidPackage) {
      const {getPlatformName, getAlreadyRunningAvd, launchAVD} = require('nightwatch-mobile-helper');
      const sdkRoot = getSdkRootFromEnv();
      const platform = getPlatformName();
      const NIGHTWATCH_AVD = desiredCapabilities.avd || 'nightwatch-android-11';
  
      return getAlreadyRunningAvd(sdkRoot, platform, NIGHTWATCH_AVD).then(async (emulatorAlreadyRunning) => {
        if (!emulatorAlreadyRunning) {
          await launchAVD(sdkRoot, platform, NIGHTWATCH_AVD);
        }
      }).catch(e => {
        const err = new Error('adb-server not running');
        err.help = [
          `Run ${Logger.colors.cyan('adb start-server')}`,
          `If still doesn't work, run ${Logger.colors.cyan('npx nightwatch-mobile-helper android')} \n`
        ];
        err.stack = false;
  ...
  async getTestsFiles() {
    ...
    for (const env of this.testEnvArray) {
      const envSettings = Settings.parse(this.userSettings, this.baseSettings, this.argv, env);
      modules[env] = await Runner.readTestSource(envSettings, this.argv);
    }

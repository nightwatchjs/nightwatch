...
class LocalTunnel {
  constructor(settings = {}) {
    ...
    this.opts = this.settings.tunnelOptions || {};
  ...
  async start() {
    if (this.local && this.key) {
      try {
        ...
        console.log("Starting of local failed", err);
      }
    }
  }
  ...
  async stop() {
    try {
      ...
      console.log("Stopping of local failed", err);
    }
  }
};
...
class CliRunner {
  ...
  async setupAsync(settings) {
    ...
    this.localTunnel = new LocalTunnel(this.test_settings);

    return this;
  }

  async startTunnel() {
    await this.localTunnel.start();
    
    return this;
  }

  async stopTunnel() {
    await this.localTunnel.stop();

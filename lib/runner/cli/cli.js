...
class CliRunner {
  ...
  isTestWorkersEnabled() {
    const modules =  Runner.readTestSource(this.test_settings, this.argv);

    return this.test_settings.testWorkersEnabled && !this.singleTestRun(modules);
  }

  singleTestRun(modules) {
    return (Array.isArray(modules) && modules.length > 1) || Utils.isString(this.argv.test) || Array.isArray(this.argv._source) && this.argv._source.length === 1;
  }
  ...
  runTests(done = null) {
    return this.runGlobalHook('before')
      ...
      .then(modules => {
        console.log('*********', this.isConcurrencyEnabled(modules));

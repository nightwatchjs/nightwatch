...
const SelectorPlaygroundServer = require('../../testsuite/selector-playground');
...
class CliRunner {
  ...
  runnerSetup() {
    ...
    if (this.validateSeletorPlaygroundCriteria()) {
      this.testRunner.playgroundServer = new SelectorPlaygroundServer();
      this.testRunner.playgroundServer.addExtensionInChromeOption(this.test_settings.desiredCapabilities);
    }
  }
  ...
  validateSeletorPlaygroundCriteria() {
    return (
      this.argv.debug && 
      !this.isConcurrencyEnabled() && 
      !this.test_settings.parallel_mode && 
      this.test_settings.desiredCapabilities.browserName === Browser.CHROME
    );
  }

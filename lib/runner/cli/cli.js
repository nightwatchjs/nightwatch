...
const {Logger, singleSourceFile, iosRealDeviceUDID, isRealIos} = Utils;
...
class CliRunner {
  ...
  initTestSettings(userSettings = {}, baseSettings = null, argv = null, testEnv = '') {
    ...
    if (isRealIos(desiredCapabilities)) {
      if (argv.deviceId) {
        ...
        const err = new Error('Real device id is niether configured or passed');
  ...
  get isMobile() {
    for (const env of this.testEnvArray) {
      const {platformName} = this.testEnvSettings[env].desiredCapabilities || {};

      if ((platformName && (platformName.toLowerCase() === 'ios' || platformName.toLowerCase() === 'android'))) {
        return true;
      }
    }
  ...
  async getTestsFiles() {
    const modules = {};
    for (const env of Object.keys(this.testEnvSettings)) {

...
class CliRunner {
  ...
  loadConfig() {
    ...
    let config = require(this.argv.config);

    try {
      // load config from package.json
      const pkgJson = require(path.resolve('./package.json'));

      if (pkgJson?.nightwatch){
        // take config from package.json as default and override with config from nightwatch config.
        const mergedPlugins = lodashUnion(pkgJson?.nightwatch.plugins, config.plugins);

        config = lodashMerge(pkgJson?.nightwatch, config);
        config.plugins = mergedPlugins;
      }
    } catch (err) {
      Logger.warn('Failed to read config from package.json:', err.message);
    }

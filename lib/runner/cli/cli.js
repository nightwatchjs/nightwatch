...
class CliRunner {
  ...
  sequentialMode() {
    // If workers argument passes with --serial mode then cosidering sequential mode will return false and tests will run concurrently but in sequence
    return this.argv.serial && !this.test_settings.testWorkersEnabled && !this.argv.workers;
  }
      ...
      .then(modules => {
        ...
        return promise.then(() => {
          if (this.sequentialMode()) {
            return this.testEnvArray.reduce((prev, env) => {
              return prev.then(async () => {
                this.processListener.testRunner.settings = this.__testEnvSettings[env];
                return await this.executeTestRunner(modules[env])
              });
            }, Promise.resolve());
          }

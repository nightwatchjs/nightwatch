...
const {Logger, singleSourceFile, iosRealDeviceUDID} = Utils;
...
class CliRunner {
  ...
  initTestSettings(userSettings = {}, baseSettings = null, argv = null, testEnv = '') {
    ...
    if (argv.deviceId && this.isRealIos) {
    ...
    return this.runGlobalHook('before', [this.test_settings])
      ...
      .then(modules => {
        ...
        if (NIGHTWATCH_AVD) {
          ...
          this.androidServer = new AndroidEmulator(NIGHTWATCH_AVD);

          promise = this.androidServer.launchEmulator();
        }
      ...
      .then(errorOrFailed => {
        ...
        if (this.test_settings.deviceUDID && !this.isRealIos) {

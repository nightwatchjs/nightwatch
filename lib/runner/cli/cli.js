...
class CliRunner {
  ...
  validateConfig() {
    ...
    this.validateChromeOptions();

    return this;
  }

  validateChromeOptions() {
    Object.keys(this.baseSettings.test_settings).map((testSettingKey) => {
      let testSetting = this.baseSettings.test_settings[testSettingKey];
      if (Utils.isObject(testSetting.desiredCapabilities) &&
        ...
        this.invalidChromeArguments = Utils.findUnquotedCliArgumentsWithWhitespaces(testSetting.desiredCapabilities.chromeOptions.args);

        if(this.invalidChromeArguments && this.invalidChromeArguments.length > 0){
          console.error('Unquoted argument value contains whitespace which may leads to errors : [ ' + this.invalidChromeArguments +
              ' ]  check your configuration to prevent future issues.');
        }
      }
    });
  }

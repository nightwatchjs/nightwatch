...
class CliRunner {
  ...
  static createDefaultConfig(destFileName) {
    ...
    let rendered = ejs.render(tplData, {
      plugins: (availablePlugins.length > 0) ? JSON.stringify(availablePlugins) : false,
      launch_url,
  ...
  loadConfig() {
    ...
    if (argvSetup.isDefault('config', this.argv.config)) {
      ...
      let localJsValue = path.resolve(usingESM ? CliRunner.CONFIG_FILE_CJS : CliRunner.CONFIG_FILE_JS);

...
const {Logger, singleSourceFile, iosRealDeviceUDID, isRealIos, isMobile, killSimulator} = Utils;
  ...
  initTestSettings(userSettings = {}, baseSettings = null, argv = null, testEnv = '') {
    ...
    if (isRealIos(desiredCapabilities)) {
      if (argv.deviceId) {
        ...
        const err = new Error('Real Device ID is neither configured nor passed');
  ...
  runTests(done = null) {
    return this.runGlobalHook('before', [this.test_settings])
      ...
      .then(modules => {
        ...
        if (!real_mobile && avd) {
          const AndroidEmulator = require('../androidEmulator.js');

  ...
  this.createFolder(function(err) {
    if (self.isDisabled()) {
      ...
      reporter.write(self.globalResults, self.options, function(err) {
        if (err) {
          console.log(Logger.getTimeStamp() + colors.yellow(format('Warning: Failed to save report file to folder: %s', self.options.output_folder)));
...
Reporter.prototype.printTotalResults = function() {
  ...
  if (this.testsFailed()) {
    ...
    console.log(
      format('\n ' + Logger.getTimeStamp() + '%s', colors.light_red('TEST FAILURE:')),
      countMessage,

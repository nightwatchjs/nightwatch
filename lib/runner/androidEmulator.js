...
module.exports = class MobileServer {
  ...
  async launchEmulator() {
    ...
    try {
      const alreadyRunningAvdEmulatorId = await getAlreadyRunningAvd(this.sdkRoot, this.platform, this.avd);

      this.emulatorId = alreadyRunningAvdEmulatorId || await launchAVD(this.sdkRoot, this.platform, this.avd);
      this.emulatorAlreadyRunning = !!alreadyRunningAvdEmulatorId;

...
TestCase.prototype.run = function () {
  ...
  this.suite
    ...
    .then(function onSuccess(response) {
      return self.suite
        .afterEach(response.results, response.errors)
        .catch(function(err) {
          // Intentionally failing tests in afterEach, i.e. calling: done(err);
          self.setFailed(err);
        });
...
TestCase.prototype.doneCallback = function(err) {
  ...
  if (Utils.isErrorObject(err) && this.suite.options.start_session) {

...
TestCase.prototype.print = function () {
  ...
  if (opts.output && opts.start_session) {
...
TestCase.prototype.getTestOutput = function (err, results) {
  var symbol, testname = this.testFn;
  ...
  if (results.time > 20) {
    args[0] += ' %s';
    args.push(Logger.colors.yellow(Utils.format('(%sms)', results.time)));
  }
...
TestCase.prototype.doneCallback = function(err) {
  ...
  if (!this.suite.options.start_session) {
    ...
    this.currentDeferred.promise.then(function(results) {
      console.log(self.getTestOutput(err, results));
    });
  }
...
TestCase.prototype.setFailed = function(err) {
  var failed = this.suite.client.results('failed');
  this.suite.client.results('failed', failed+1);
  this.suite.client.results('stackTrace', err.stack);
};

...
class Concurrency extends EventEmitter {
  ...
  runMultipleTestWorkers(envs, modules, args, availColors) {
    ...
    let remaining = modules.length * envs.length;
    maxWorkerCount = remaining > maxWorkerCount? maxWorkerCount : remaining; 
    ...
    return new Promise((resolve, reject) => {
      Concurrency.buildProcessQueue(maxWorkerCount, modules, envs, (env, modulePath, index, next) => {
        let outputLabel = Utils.getModuleKey(modulePath, this.settings.src_folders, modules);
        let childProcess = this.createChildProcess(outputLabel, args, ['--test', modulePath, '--env', env, '--test-worker'], index);

        return this.runChildProcess(childProcess, `${env} : ${outputLabel}`, availColors, 'workers')
          .then(_ => {
            remaining -=1;
   ...
   * @param {function} fn
   */
  static buildProcessQueue(concurrency, modules, envs, fn) {
    const maxWorkers = Math.min(concurrency, modules.length * envs.length);

    const queue = [];
    envs.forEach((env) => {
      queue.push(...(modules.map((module) => {
        return {
          env,
          module
        };

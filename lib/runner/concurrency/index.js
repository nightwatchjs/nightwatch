...
const WorkerPool = require('./worker-process');
const {Logger} = Utils;
...
class Concurrency extends EventEmitter {
  ...
  runMultipleTestWorkers(modules, args, availColors) {
    ...
    const workerPool = new WorkerPool([], null, this.settings);
    ...
    modules.forEach(module => {
      let outputLabel = Utils.getModuleKey(module.module, this.settings.src_folders, modules);
      const workerArgv = {...this.argv};
      workerArgv._source = [module.module];
      workerArgv.env = module.env;
   ...
   * 
   */
  // createWorkerPool(args = [], extraArgs) {
  //   this.childProcessOutput[label] = [];
  //   const workerArgs = args.slice().concat(extraArgs);
  //   const workerPool = new workerPool(workerArgs, null, this.settings);
  // }

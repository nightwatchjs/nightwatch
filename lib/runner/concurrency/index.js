...
class Concurrency extends EventEmitter {
  ...
  runChildProcesses(envs, modules) {
    ...
    if (this.settings.testWorkersEnabled) {
      ...
      if (envs.length > 0) {
        envs.forEach((env) => {
          const filteredModules = this.applyFilters(modules, env);
          const jobList = filteredModules.map((module) => {
            return {
              env,
              module
            };
  ...
  getTestWorkersCount() {
    ...
    if (isObject(test_workers) && isNumber(test_workers.workers)) {
      if (workers < test_workers) {
        // eslint-disable-next-line no-console
        console.warn(`Number of max workers is set to ${test_workers.workers} while the number of cpu cores is ${workers}. This can cause performance issues...\n`);
      }

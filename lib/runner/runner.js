...
class Runner {
  ...
  static checkTestSource(modules, fullPaths, settings) {
    if (modules && modules.length === 0) {
      ...
      if (this.settings.tag_filter) {
        detailed.push(`- using tags: ${this.settings.tag_filter}`);
      }

      if (this.settings.filter) {
        detailed.push(`- using filter: ${this.settings.filter}`);
      }

      if (this.settings.exclude) {
        detailed.push(`- using exclude match: ${this.settings.exclude}`);
      }
  ...
  static create(source, settings, argv, addtOpts) {
    switch (settings.test_runner.type) {
      case Runner.DEFAULT_RUNNER:
        return new DefaultRunner(source, settings, argv, addtOpts);

      case Runner.MOCHA_RUNNER:
        return new MochaRunner(source, settings, argv, addtOpts);
    }

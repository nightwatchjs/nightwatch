...
const fs = require('fs');
const {Logger, getFreePort} = require('../../utils');
...
class BaseWDServer {
  ...
  get defaultPort() {
    return BaseWDServer.DEFAULT_PORT;
  }
  ...
  async setCliArgs() {
    ...
    const {findFreePort} = this.options;
    if (findFreePort) {
      this.settings.port = await getFreePort(this.settings.host);
    }

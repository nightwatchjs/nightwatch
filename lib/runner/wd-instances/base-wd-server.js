...
class BaseWDServer {
   ...
   * @return {number}
   */
  static get processCreatedTimeoutMs() {
    return 120000;
  }
  ...
  constructor(settings) {
    ...
    this.statusPollInterval = settings.status_poll_interval || BaseWDServer.statusPollInterval;
    this.processCreatedTimeoutMs = settings.driver_start_timeout || BaseWDServer.defaultGlobalStartTimeout;
  ...
  get initialCheckProcessDelay() {
    return this.settings.check_process_delay || 0;
  }
  ...
  pingStatus() {
    Logger.info(`Checking ping status:  ${this.settings.host}:${this.settings.port}${this.statusCheckEndpoint}`);
  ...
  checkPingStatus() {
    if (this.statusPingTries < this.maxStatusPollTries) {
      ...
      this.promiseStarted.reject(this.createError(`Timeout while trying to connect to ${this.serviceName} `+
          ...
          `Number of retries: ${this.statusPingTries}`));
    }
  }

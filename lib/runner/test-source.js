...
const {Logger, validExtensions, tsFileExt, jsFileExt, singleSourceFile, isRerunFailedEnabled, getRerunFailedFile} = Utils;
...
class TestSource {
  ...
  getTestSourceForRerunFailed(argv) {
    const rerunJsonFile = getRerunFailedFile(argv);
  
    try {
      const {modules = {}} = require(rerunJsonFile);
      const testsource = [];
      ...
      // Checking if error is not from 
      if (!err.detailedErr) {
        err.message = 'Rerun Failed Tests: Invalid Json reporter.';

        err.showTrace = false;
        err.detailedErr = 'Please set env variable NIGHTWATCH_RERUN_FAILED_FILE with valid Json reporter path.';
      }
   ...
   */
  getSource() {
    if (isRerunFailedEnabled(this.argv) && Concurrency.isMasterProcess()) {

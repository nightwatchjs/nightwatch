...
const {RerunJsonReporter} = require('../reporter/reporters/rerunJson');
...
class TestSource {
  ...
  getTestSourceForRerunFailed() {
    ...
    const rerunJsonFile = RerunJsonReporter.getRerunFailedFile(rerun_report_file_path);
   ...
   */
  getSource() {
    if (RerunJsonReporter.isRerunFailedEnabled(this.argv) && Concurrency.isMasterProcess()) {

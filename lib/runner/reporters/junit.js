...
class JUnitReporter {
  ...
  writeReport(moduleKey, data) {
    ...
    let filename = path.join(output_folder, `${opts.filename_prefix}${moduleName}.xml`);
  ...
  write() {
    ...
    return this.loadTemplate()
      .then(data => {
        let moduleKey = keys.shift();

        return new Promise(function createReport(resolve, reject) {
          this.writeReport(moduleKey, data).then(err => {
            if (err) {
              return reject(err);
            }

...
class AnalyticsCollector {  
  constructor() {
    // incase settings are not updated, disable analytics
    this.settings = {
      analytics: {
        enabled: false
      }
  ...
  updateSettings(settings) {
    ...
    try {
      ...
      this.queueLength = stat.size; // the actual value doesnot matter
    } catch (e) {;}
  }
  ...
  async logAnalyticsEvents(data) {
    ...
    return new Promise((resolve, reject) => {
      writeFn(logfile, JSON.stringify(data), function (err) {
        if (err) {
          this.logger.error('Failsed to log analytics data');
        } 
        resolve();
      });
    });

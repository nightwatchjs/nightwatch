...
class AnalyticsCollector {  
  ...
  updateSettings(settings) {
    ...
    if (this.settings.usage_analytics.enabled) {
      try {
        ...
        this.queueLength = stat.size / 400; // the actual value doesn't matter
      } catch (e) {
      // ignore
      }
    }
  ...
  event(name, parameters = {}) {
    ...
    parameters['env_language'] = getLanguage();
    ...
    return this.__addToQueue(name, parameters);
  }
 
  exception(error, location = 'lib/runner/process-listener.js#uncaught') {
    try {
      const parameters = {
        env_os: buildUserAgentString(),
        env_language: getLanguage(),
      ...
      this.__addToQueue('nw_exception', parameters);
    } catch (e) {
      // Ignore
    }
  }
  ...
  async __flush() {
    ...
    try {
      ...
      const pendingLogs = fs.readFileSync(logfile, 'utf8');
  ...
  __addToQueue(eventType, parameters) {
    ...
    const writeLogPromise = this.__logAnalyticsEvents({name: eventType, params: parameters});
  ...
  async __logAnalyticsEvents(data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      writeFn(logfile, JSON.stringify(data) + '\n', function (err) {
        if (err) {

...
class AnalyticsCollector {  
  ...
  async flush() {
    ...
    Logger.info(`analytics flush queue size: ${pending}`);
  ...
  async send(data) {
    ...
    return new Promise((resolve, reject) => {
      const path = `/debug/mp/${data.length > 1? 'batch': 'collect'}?api_secret=91SrwdZhS-md0N2C-UaY1A&measurement_id=${this.settings.analytics.trackingId}`; 

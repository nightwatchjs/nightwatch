...
class AssertionLoader extends BaseCommandLoader {
  ...
  static createInstance(nightwatchInstance, AssertionModule, moduleArgs) {
  ...
  createWrapper(abortOnFailure) {
    if (this.module) {
      ...
      this.commandFn = function commandFn(...args) {
        return this.updateElementSelector(args)
          .then(elementResult => {
            if (elementResult) {
              args[0] = elementResult;
            }

            const instance = AssertionLoader.createInstance(this.nightwatchInstance, this.module.assertion, args);

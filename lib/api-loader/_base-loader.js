...
class BaseLoader extends EventEmitter {
  ...
  validateMethod(parent) {
    ...
    */
    ...
    if (this.nightwatchInstance.isApiMethodDefined(this.commandName, namespace)) {
      let error = new Error(`The ${this.type} ${this.namespace || ''}.${this.commandName}() is already defined.`);
      Reporter.registerTestError(error);
      throw error;
    }
  ...
  define(parent = null) {
    ...
    let args = [function commandFn(...args) {
      ...
      this.commandQueue.add(commandName, command, this, args, originalStackTrace);

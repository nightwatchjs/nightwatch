...
class ElementCommandLoader extends BaseCommandLoader {
  ...
  constructor(nightwatchInstance, protocolCommands) {
    super(nightwatchInstance);

    this.__strategy = null;
    this.__value = null;
    this.__callback = null;
    this.__protocolCommands = protocolCommands;
  }
    ...
    this.transport
      ...
      .then(result => {
        ...
        if (elementId.length > 1) {
          ...
          let message = `Found ${elementId.length} elements for selector: "${this.instance.selector}".`;
          console.warn(message);
        }

        let protocolAction = ElementCommandLoader.elementApiMappings[this.commandName];
        this.args.unshift(elementId[0]);

        return this.protocolCommands[protocolAction](...this.args);
      })
      .then(result => {
      ...
      .catch(result => {
        ...
        try {
          this.callback.call(this.api, callbackResult);
        } catch (err) {
          this.emit('error', err);

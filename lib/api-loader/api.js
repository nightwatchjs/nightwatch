...
class ApiLoader {
  ...
  loadElementCommands(parent = null) {
    ...
    commandFiles.forEach(file => {
      const loader = new ElementCommandLoader(this.nightwatchInstance, this.protocolInstance);

      loader.isUserDefined = false;
      loader.loadModule(ApiLoader.ElementCommandsPath, file).createWrapper().define(parent);
    });

    // loads element commands define in bulk, which are mainly wrappers over existing protocol actions

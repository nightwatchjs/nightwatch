...
class ApiLoader {
  ...
  loadClientCommands(parent = null, sourcePath = ApiLoader.CommandsFiles) {
    ...
    if (CommandLoader.isFileNameValid(sourcePath)) {
      const commandDefinitions = require(path.join(__dirname, sourcePath))(this.nightwatchInstance);
      const entries = Object.keys(commandDefinitions);
  ...
  loadPageObjectsFromFolder(sourcePath = null, namespace = []) {
    ...
    pageFiles.forEach(file => {
      if (fs.lstatSync(path.join(sourcePath, file)).isDirectory()) {
        ...
        ns.push(file);
        this.loadPageObjectsFromFolder(pathFolder, ns);

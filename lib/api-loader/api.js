...
class ApiLoader {
  ...
  loadCustomCommands(parent = null) {
    if (this.nightwatchInstance.options.custom_commands_path) {
      let fullPath = path.resolve(this.nightwatchInstance.options.custom_commands_path);
      this.loadCommandsFromFolder(parent, fullPath, true);
    }
  ...
  createStaticAssertion(prop, abortOnFailure) {
    ...
    return function assertFn(...args) {
      ...
      this.commandQueue.add(prop, function() {
        NightwatchAssertion.create(assertion.passed, {
          actual: assertion.actual,
          expected: assertion.expected
        }, assertFn, assertion.message, abortOnFailure, assertFn.stackTrace).run(this.reporter, assertion);
      }.bind(this), this.api, [], assertFn.stackTrace);
  ...
  loadCustomAssertions(parent = null) {
    if (this.nightwatchInstance.options.custom_assertions_path) {
      let fullPath = path.resolve(this.nightwatchInstance.options.custom_assertions_path);
      this.loadAssertions(parent, fullPath, true);
    }
  ...
  loadPageObjects(sourcePath = null) {
    ...
    sourcePath = sourcePath || path.resolve(this.nightwatchInstance.options.page_objects_path);

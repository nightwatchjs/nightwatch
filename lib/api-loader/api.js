...
module.exports = new (function() {
  ...
  class ApiLoader {
     ...
     */
    loadCommandsFolder(parent = null, sourcePath, isUserDefined = false) {
      if (sourcePath && Array.isArray(sourcePath)) {
        sourcePath.forEach(item => {
          this.loadCommandsFolder(parent, item, isUserDefined);
        });
        return;
      }
      ...
      for (let i = 0, len = commandFiles.length; i < len; i++) {
        ...
        loader.loadModule(sourcePath, commandFiles[i]).createWrapper().define(parent);
      }
    }

    loadCommands(parent = null, sourcePath = CommandsFiles) {
      if (Array.isArray(sourcePath)) {
        sourcePath.forEach(item => {
          this.loadCommands(parent, item);
        });
        return this;
      }
      ...
      if (CommandLoader.isFileNameValid(sourcePath)) {
        ...
        this.loadCommandsFolder(parent, path.join(__dirname, sourcePath), false);
      }
    ...
    loadCustomCommands(parent = null) {
      if (this.nightwatchInstance.options.custom_commands_path) {
        this.loadCommandsFolder(parent, this.nightwatchInstance.options.custom_commands_path, true);
      }
    ...
    loadAssertions(parent = null, folder = AssertionsBasePath, isUserDefined = false) {
    ...
    loadCustomAssertions(parent = null) {
      this.loadAssertions(parent, this.nightwatchInstance.options.custom_assertions_path, true);
    }
    ...
    loadExpectAssertions(parent = null) {
      ...
      expectLoader.createWrapper().define(parent);
    ...
    loadPageObjects(sourcePath = null) {
      ...
      for (let i = 0, len = pageFiles.length; i < len; i++) {
        ...
        loader.loadModule(sourcePath, pageFiles[i]).define(parent => {
          if (this.nightwatchInstance.startSessionEnabled) {
            this.loadElementCommands(parent)
              .loadCommands(parent)
          ...
          this.loadAssertions(parent)
            .loadCustomCommands(parent)
            .loadCustomAssertions(parent);

...
const {GlobalHookRunStarted, GlobalHookRunFinished, TestSuiteStarted, TestSuiteFinished, HookRunStarted, HookRunFinished, TestRunStarted, TestRunFinished} = DEFAULT_RUNNER_EVENTS;
...
class TestSuite {
  ...
  runHook(hookName) {
    ...
    if (NightwatchEventHub.isAvailable && this.context.hasHook(hookName)) {
  ...
  runGlobalHook(hookName) {
    ...
    if (NightwatchEventHub.isAvailable && globals[hookName]) {
      NightwatchEventHub.emit({

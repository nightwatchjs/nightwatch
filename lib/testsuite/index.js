...
class TestSuite {
  ...
  initCommon(opts = {}) {
    ...
    this.createContext(opts);
    this.setSuiteName();
    this.setRetries();
  }

  init(opts = {}) {
    this.initCommon(opts);
    this.createClient();
    this.setupHooks();
  ...
  createContext({context = null, reloadModuleCache = false, suiteTitle = null, attributes = {}} = {}) {
    if (context) {
    ...
    this.context
      ...
      .init({usingMocha: this.usingMocha, suiteTitle})
  ...
  addPropertiesToGlobalScope() {
    ...
    Object.defineProperty(global, 'element', {
      ...
      value: function(locator) {
        return ElementGlobal.element({locator, testSuite: this});
      }.bind(this),
      writable: false
    });
  ...
  createClient(client = null) {
    ...
    if (this.settings.sync_test_names) {
      settings.desiredCapabilities.name = this.suiteName;
    }

    if (this.context.getDesiredCapabilities()) {
      Object.assign(settings.desiredCapabilities, this.context.getDesiredCapabilities());
    }
    ...
    this.client = Nightwatch.client(settings, this.reporter, this.argv);
  ...
  createSession() {
    ...
    return this.client.createSession(this.argv).catch(err => {
  ...
  runTestSuite() {
    return this.startTestSuite()
      .then(() => this.testSuiteRunFn())
      .then((result) => this.onTestSuiteFinished(result));
  }

  onTestSuiteFinished(failures = false) {
    return this.runGlobalHook('afterEach')
      ...
      .then(failedResult => {
        if (this.shouldRetrySuite(failures)) {
          return this.retrySuite();
        }

        return failures || failedResult;
      })
      .then(failures => {
        let testFailed = failures || !this.reporter.allTestsPassed;

        return this.stopSession(testFailed);
      });
  }

  async stopSession(testFailed) {
    try {
  ...
  testSuiteFinished(failures) {
    ...
    this.transport.testSuiteFinished(failures);
    this.transport.sessionFinished(failures ? 'FAILED' : '');

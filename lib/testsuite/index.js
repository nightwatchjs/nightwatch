...
class TestSuite {
  ...
  runHook(hookName) {
    ...
    if (this.context.hasHook(hookName)) {
      NightwatchEventHub.emit(TestSuiteHook[hookName].started, {
    ...
    return this.handleRunnable(hookName, () => {
      ...
      if (this.context.hasHook(hookName)) {
        NightwatchEventHub.emit(TestSuiteHook[hookName].finished, {
          testResults: this.reporter.testResults.export
        });
      }
  ...
  runGlobalHook(hookName) {
    ...
    if (globals[hookName]) {
      NightwatchEventHub.emit(GlobalHook[hookName].started, {
        testResults: this.reporter.testResults.export
      });
    }
    ...
    return this.handleRunnable(hookName, async () => {
      ...
      if (globals[hookName]) {
        NightwatchEventHub.emit(GlobalHook[hookName].finished, {
          testResults: this.reporter.testResults.export
        });
      }
  ...
  async startTestSuite() {
    NightwatchEventHub.emit(TestSuiteHook.started, {
      testResults: this.reporter.testResults.export
    });
  ...
  async testSuiteFinished(failures) {
    ...
    NightwatchEventHub.emit(TestSuiteHook.finished, {
      testResults: this.reporter.testResults.export
    });

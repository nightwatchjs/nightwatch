        ...
        this.api.saveScreenshot(fileNamePath, (result, err) => {
          if (!err && this.transport.isResultSuccess(result))  {
            ...
            if (assertions.length > 0) {
              ...
              const {folder_format, output_folder} = this.settings.reporter_options;
              const destFolder = path.join(output_folder, Utils.getFolderPrefix(folder_format), 'nightwatch-html-report');
              const relativeFilePath = path.relative(destFolder, fileNamePath);

              if (currentAssertion) {
                currentAssertion.screenshots = currentAssertion.screenshots || [];
                currentAssertion.screenshots.push(relativeFilePath);
              }

              if (currentCommand) {
                currentCommand.screenshot = relativeFilePath;
              }
            }
          } else {

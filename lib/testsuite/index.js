...
const {By, Key, locateWith, withTagName, Browser} = require('selenium-webdriver');
...
class TestSuite {
  constructor({modulePath, modules, settings, argv, usingMocha = false, addtOpts = {}, playgroundServer = null}) {
    ...
    this.playgroundServer = playgroundServer;
  }
  ...
  async initClient({initialize = true} = {}) {
    ...
    if (this.playgroundServer) {
      this.playgroundServer.setClient(this.client);
      this.playgroundServer.addExtensionInChromeOption();
    }
  }

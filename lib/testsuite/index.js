...
const {Logger, Screenshots, alwaysDisplayError} = require('../utils');
...
class TestSuite {
  ...
  runHook(hookName, possibleError) {
    Logger.log(`${Logger.colors.green('→')} Running [${hookName}]:`);

    return this.handleRunnable(hookName, () => {
      return this.hooks[hookName].run(this.client, null, possibleError);
    }).then(result => {
  ...
  runCurrentTest(testName) {
    ...
    return this.createSession()
      ...
      .then(possibleError => {
        ...
        return this.runHook('afterEach', possibleError)

...
class TestSuite extends EventEmitter {
  ...
  get skipTestcasesOnFail() {
    let definedValue = this.context.getSkipTestcasesOnFail();
    if (definedValue === undefined && this.settings.skip_testcases_on_fail === undefined && this.context.unitTestsMode) {
      definedValue = false;
    } else if (definedValue === undefined && !this.context.unitTestsMode) {
      // true by default when not running unit tests
      definedValue = this.settings.skip_testcases_on_fail;
    }
  ...
  createClient(client = null) {
    ...
    this.reporter = new Reporter(this.context.tests, this.suiteRetries, this.settings, {
    ...
    this.client = Nightwatch.client(this.settings, this.reporter);
  ...
  destroySession() {
    this.client.session.clearQueue();
  }

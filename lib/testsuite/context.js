...
class Context {
   ...
   * @return {*}
   */
  callAsync(fnName, api, expectedArgs = 2, isGlobal, done = function() {}) {
    let fnAsync = Utils.makeFnAsync(expectedArgs, this.module[fnName], this.module);
    let args = this.getHookFnArgs(done, api, isGlobal, expectedArgs);
   ...
   * @return []
   */
  getHookFnArgs(done, api, isGlobal, expectedArgs) {
    let args = [isGlobal && expectedArgs === 1 ? done : api];
    if (expectedArgs === 2) {
      args.push(done);
    }
   ...
   * @return {*}
   */
  invokeMethod(key, api, argsCount, isGlobal, done) {
    switch (argsCount) {
      case 2:
      case 1:
        return this.callAsync(key, api, argsCount, isGlobal, done);
  ...
  setReportKey() {
    ...
    let srcFolders = this.settings.src_folders;

...
module.exports = function(Nightwatch) {
  ...
  /**
   * Set the amount of time the driver should wait when searching for elements. When searching for a single element, the driver should poll the page until an element is found or the timeout expires, whichever occurs first.
   ...
   * @param {function} [callback] Optional callback function to be called when the command finishes.
   * @api protocol
   * @returns {*} value|The script result.
   */
  ...
  Actions.window = function(method, handleOrName, callback) {
    ...
    switch (method) {
      ...
      case 'DELETE':
        var options = {
          path : '/session/' + Nightwatch.sessionId + '/window',
          method : 'DELETE'
        };
  ...
  Actions.status = function(callback) {
    return getRequest('/status', callback);
  };

...
exports.assertion = function(selector, attribute, expected, msg) {
  ...
  var MSG_ELEMENT_NOT_FOUND = 'Testing if attribute %s of <%s> contains %s. ' +
    'Element or attribute could not be located.';

  this.message = msg || util.format('Testing if attribute %s of <%s> contains: %s.', attribute, selector, expected);
  ...
  this.failure = function(result) {
    ...
    if (failed) {
      this.message = msg || util.format(MSG_ELEMENT_NOT_FOUND, attribute, selector, expected);
    }
    return failed;
  };

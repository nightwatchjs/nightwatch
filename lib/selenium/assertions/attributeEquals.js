...
module.exports = function(selector, attribute, expected, msg) {
  ...
  var MSG_ASSERTION = 'Testing if element <%s> has attribute: %s = "%s"';
  ...
  return this.api.getAttribute(selector, attribute, function(result) {
    ...
    if (result === false || result && result.status === -1) {
      passed = false;
      defaultMsg = util.format(MSG_ELEMENT_NOT_FOUND, selector, attribute);
    } else {
      passed = result.value === expected;
      value = result.value;
      defaultMsg = util.format(MSG_ASSERTION, selector, attribute, expected);
    }

    msg = msg || defaultMsg;
    client.assertion(passed, value, expected, msg, self.abortOnFailure);
  });
};
*/

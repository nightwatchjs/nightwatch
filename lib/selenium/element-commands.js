...
module.exports = function(client) {
   ...
   *
   * A json file with available key code UTF-8 characters can be found here.
   *
   * ```
   * // send some simple text to a text input
   * this.demoTest = function (browser) {
   *   browser.setValue('input[type=text]', 'nightwatch');
   * };
   * // send some text to a text input and hit enter.
   * var Keys = require('../../lib/keys.json');
   * this.demoTest = function (browser) {
   *   browser.setValue('input[type=text]', ['nightwatch', Keys.ENTER]);
   * };
   * ```
   *

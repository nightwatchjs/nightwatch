...
CommandAction.prototype.command = function(cssSelector, milliseconds, callback, msg) {
  ...
  if (typeof milliseconds != 'number') {
    throw new Error('waitForElementInVisible expects second parameter to be number; ' +
...
CommandAction.prototype.isInVisible = function() {
  var self = this;
  var el = Protocol.actions.elementIdDisplayed.call(this.client, this.element, function(result) {
    ...
    if (result.status == 0 && result.value === false) {
      self.cb(result);  
      var msg = "Element <" + self.selector + "> was invisible after " + (now - self.startTimer) + " milliseconds.";
    ...
    if (now - self.startTimer < self.ms) {
      ...
      self.cb(false);
      var msg = "Element <" + self.selector + "> was still visible after " + self.ms + " milliseconds.";

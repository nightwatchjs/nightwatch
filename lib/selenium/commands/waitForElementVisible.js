...
/**
 * Waits a given time in milliseconds for an element to be visible in the page
 * before performing any other commands or assertions. If the element fails to
 * be present and visible in the specified amount of time, the test fails.
 *
 * ```
...
WaitForElementVisible.prototype.elementVisible = function(result, now) {
  var msg = 'Element <' + this.selector + '> was visible after ' +
...
WaitForElementVisible.prototype.elementNotVisible = function(result, now) {
  ...
  var msg = 'Timed out while waiting for element <' + this.selector +
    '> to be visible for ' + this.ms + ' milliseconds.';
  return this.fail(result, 'not visible', this.expectedValue, msg);
};

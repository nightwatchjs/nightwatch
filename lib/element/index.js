...
class Element {
  ...
  setProperties(definition, options) {
    ...
    const locateStrategyFromNightwatchConfig = options.parent ? options.parent.client.locateStrategy : undefined;
    
    this.locateStrategy = definition.locateStrategy || options.locateStrategy || locateStrategyFromNightwatchConfig || LocateStrategy.getDefault();
  ...
  toString() {
    ...
    if (!this.name) { // inline (not defined in page or section)
      ...
      if (this.elementId) {
        return `WebElement: ${this.elementId}`;
      }
  ...
  static createFromSelector(value, using) {
    ...
    if (value && value.elementId && value.sessionId) {
      return value;
    }

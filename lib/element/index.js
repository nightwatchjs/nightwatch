...
class Element {
  ...
  setProperties(definition, options) {
    ...
    } else if (definition.driver_ && definition.id_) {
  ...
  static createFromSelector(value, using) {
    ...
    if (value.elementId && value.sessionId) {
      return value;
    }
    ...
    if (value.webElementLocator instanceof By) {
      const {webElementLocator} = value;
      definition = {
        selector: webElementLocator.value,
        locateStrategy: webElementLocator.using
      };
    } else {
      options = {

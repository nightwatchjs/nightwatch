...
class ElementCommand extends EventEmitter {
  ...
  setupExecutor() {
    const staleCondition = (result) => this.transport.staleElementReference(result);
    ...
    this.executor
      ...
      .queueAction({
        ...
        successHandler: (result) => this.protocolActionHandler(result),
        errorHandler: (err) => {
  ...
  complete(err, response) {
    console.log('COMPLETE', err instanceof Error, response)
    console.trace()

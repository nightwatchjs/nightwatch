...
class ElementCommand extends EventEmitter {
  ...
  get supressNotFoundErrors() {
    return this.__supressNotFoundErrors;
  }

  set supressNotFoundErrors(value) {
    this.__supressNotFoundErrors = Utils.convertBoolean(value);
  }
  ...
  constructor({nightwatchInstance, commandName, args, requireValidation = true} = {}) {
    ...
    this.supressNotFoundErrors = false;
  ...
  setOptionsFromSelector() {
    if (Utils.isObject(this.selector)) {
      const {
        abortOnFailure, retryInterval, message, timeout, locateStrategy, supressNotFoundErrors, retryAction
      } = this.selector;
      ...
      if (!Utils.isUndefined(supressNotFoundErrors)) {
        this.supressNotFoundErrors = supressNotFoundErrors;
      }
  ...
  noSuchElementError() {
    ...
    return new NoSuchElementError({element, ms, abortOnFailure});
  }
}

class NoSuchElementError extends Error {
  constructor({element, ms, abortOnFailure}) {
    super();

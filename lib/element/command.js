...
class ElementCommand extends EventEmitter {
  ...
  noSuchElementError(err) {
    const {element, ms, abortOnFailure, stackTrace:stack} = this;
    const {retries} = err;

    return new NoSuchElementError({element, ms, abortOnFailure, retries, stack});
  }
}

class NoSuchElementError extends Error {
  constructor({element, ms, abortOnFailure, retries, stack}) {
    super();

...
class LocateElement {
  ...
  resolveElementRecursively({element}) {
    return this.findElement({element}).then(response => {
      ...
      const WebdriverElementId = response.result && response.result.WebdriverElementId || null;
      const result = {
        selector,
        locateStrategy,
        name,
  ...
  async findElement({element, commandName, id, transportAction = 'locateMultipleElements', returnSingleElement = true}) {
    ...
    const result = await this.executeProtocolAction({element, commandName, id, transportAction});
   ...
   * }})
   */
  executeProtocolAction(opts = {}, currentRetry = 0) {
    const {id, element, transportAction, commandName} = opts;
    const args = {
      id
    };
    ...
    return this.transport.executeProtocolAction(transportAction, args)
      ...
      .then(result => {
        ...
        // }
        //
        // if (!this.transport.isRetryableElementError(result)) {
        //   this.reporter.registerTestError(result.error);
        ...
        //
        // if (currentRetry < 3) {
        //   return this.executeProtocolAction(opts, ++currentRetry);
        // }

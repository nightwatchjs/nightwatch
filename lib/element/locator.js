...
class LocateElement {
  ...
  findElements(element) {
    ...
    return this.transport.executeProtocolAction('locateMultipleElements', [element.locateStrategy, element.selector])
      .then(result => {
      ...
      .then(result => {
        ...
        let elementResult = this.resolveElement(result, element, true);
  ...
  callElementLocateAction(element, elementAction, args, multipleElements = false) {
    return this.transport.executeProtocolAction(elementAction, args)
      .then(result => {
        if (this.transport.isResultSuccess(result) && Element.requiresFiltering(element)) {
          return this.filterElements(element, result);
        }
  ...
  findElementFromParent(parentElementId, element) {
    return this.callElementLocateAction(element, 'locateMultipleElementsByElementId', [

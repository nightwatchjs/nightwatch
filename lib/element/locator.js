...
class LocateElement {
   ...
   * @return {Promise}
   */
  async findElement({element, commandName, id, transportAction, returnSingleElement}) {
    if (element && element.webElement) {
      const webElement = await element.convertWebElement();
      webElement.value = this.transport.toElement(webElement.value);

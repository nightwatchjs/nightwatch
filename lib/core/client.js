...
const namespacedApi = require('../namespaced-api.js');
...
class NightwatchClient extends EventEmitter {
  ...
  setNamespacedApiMethod(key, ...args) {
    let fn;
    let context = namespacedApi;
    let namespace;

    if (args.length === 1) {
      fn = args[0];
    } else if (args.length === 2) {
      namespace = typeof args[0] == 'string' ? context[args[0]] : args[0];
      if (namespace) {
        context = namespace;
      }
   ...
   * @return {boolean}
   */
  isApiMethodDefined(key, namespace, api) {
    api = api || this.__api;
    if (namespace) {

...
class NightwatchClient extends EventEmitter {
  ...
  /**
   * add sinon api methods (stub, spy, mocks)
   */
  addSinonApis() {
    this.setApiMethod('stub', sinon.stub);
    this.setApiMethod('spy', sinon.spy);
    this.setApiMethod('mock', sinon.mock);
    this.setApiMethod('restoreMocks', sinon.restore);
  }

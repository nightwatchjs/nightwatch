...
const LoggerColors = require('../util/logger.js').colors;
...
class Reporter {
  ...
  printGlobalError(err, outputEnabled) {
    let stackTrace = '';
    if (err.stack) {
  ...
  handleTestException(err, outputEnabled = true, terminateFn = function() {}) {
    ...
    if (typeof err.actual != 'undefined' && typeof err.expected != 'undefined') {
      ...
      firstLine += ` - expected ${LoggerColors.green(expectedMsg)} but got: ${LoggerColors.red(actualMsg)}`;
    }
  ...
  printResult(elapsedTime, startSessionEnabled) {
    ...
    if (ok && this.results.passed > 0) {
      console.log(`\n ${LoggerColors.green('OK.')} ${LoggerColors.green(this.results.passed)} assertions passed. (${Utils.formatElapsedTime(elapsedTime, true)})`);
    } else if (ok && this.results.passed === 0) {
      if (startSessionEnabled) {
        console.log(LoggerColors.green('No assertions ran.'));
      }
    } else {
      let failureMsg = this.getFailureMessage();
      console.log(`\n ${LoggerColors.red('FAILED: ')} ${failureMsg} (${Utils.formatElapsedTime(elapsedTime, true)})`);
    }
  }
  ...
  getFailureMessage() {
    ...
    if (this.results.failed > 0) {
      failureMsg.push(`${LoggerColors.red(this.results.failed)} assertions failed`);
    }

    if (this.results.errors > 0) {
      failureMsg.push(`${LoggerColors.red(this.results.errors)} errors`);
    }

    if (this.results.passed > 0) {
      failureMsg.push(`${LoggerColors.green(this.results.passed)} passed`);
    }

    if (this.results.skipped > 0) {
      failureMsg.push(`${LoggerColors.blue(this.results.skipped)} skipped`);
    }

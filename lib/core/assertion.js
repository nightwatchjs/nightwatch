...
module.exports = new (function() {
  ...
  BaseAssertion.prototype._scheduleAssertion = function() {
    ...
    return this.command(function(result) {
      ...
      var screenshots = self.screenshots;

      self.client.assertion(passed, value, expected, { message: message, screenshots: screenshots }, self.abortOnFailure, self._stackTrace);
      self.emit('complete');
    });
  };
   ...
   * @param {Object|String} assertionOutput
   * @param {Boolean} abortOnFailure
   * @param {String} originalStackTrace
   */
  this.assert = function assert(passed, receivedValue, expectedValue, assertionOutput, abortOnFailure, originalStackTrace) {
    if (!initialized) {
    ...
    var stacktrace = '';
    var fullMsg = '';
    var message = assertionOutput.message || assertionOutput;
    var screenshots = assertionOutput.screenshots || [];

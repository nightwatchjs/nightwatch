...
module.exports = new (function() {
  ...
  this.assert = function assert(passed, receivedValue, expectedValue, message, abortOnFailure, originalStackTrace) {
    ...
    if (passed) {
      ...
      err.stack = buildStackTrace(originalStackTrace, client.options.start_session ? null : Logger.getTimeStamp() + ' AssertionError: ' + message);
      fullMsg = message;
      if (client.options.output && client.options.detailed_output) {
        var logged = Logger.getTimeStamp() + ' ' + Logger.colors.red(failSymbol);

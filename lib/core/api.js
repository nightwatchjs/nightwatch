...
module.exports = new (function() {
  ...
  var custom_assertions_filter;
  var page_objects_path;
  var page_objects_filter;
  var fileExtRegExp = /\.[^/]*?$/;
  ...
  function loadAssertionFiles(dirPath, parent, abortOnFailure) {
    var assertFiles = fs.readdirSync(dirPath);

    assertFiles.forEach(function(file){
        if (minimatch(file, custom_assertions_filter)) {
            var fileExt = file.match(fileExtRegExp)[0];
            var name = path.basename(file, fileExt);
            var assertionFn = require(path.join(dirPath, file));

            createAssertion(name, assertionFn, abortOnFailure, parent);
        }
    });
  }
  ...
  function loadCustomCommands(dirpath, parent) {
    ...
    commandFiles.forEach(function(file) {
      if (minimatch(file, custom_commands_filter)) {
  ...
  function loadPageObjects() {
    ...
    pageFiles.forEach(function(file) {
        if (minimatch(file, page_objects_filter)) {
            var fileExt = file.match(fileExtRegExp)[0];
            var name = path.basename(file, fileExt);
            var pageFnOrObject = require(path.join(absPath, file));
            addPageObject(name, pageFnOrObject, client.api, client.api.page);
        }
    });
  ...
  this.init = function(c) {
    var custom_files_filter = c.options.custom_files_filter || '*.js';
    client = c;
    custom_commands_path = c.options.custom_commands_path;
    custom_commands_filter = c.options.custom_commands_filter || custom_files_filter;
    custom_assertions_path = c.options.custom_assertions_path;
    custom_assertions_filter = c.options.custom_assertions_filter || custom_files_filter;
    page_objects_path = c.options.page_objects_path;
    page_objects_filter = c.options.page_objects_filter || custom_files_filter;
    return this;
  };

...
class AsyncTree extends EventEmitter{
  ...
  constructor({compatMode, foreignRunner}) {
    super();
  ...
  async traverse(err, possibleError) {
    this.possibleError = this.possibleError || possibleError;
  ...
  async runChildNode(node) {
    ...
    if (abortOnFailure && !this.possibleError) {
      this.empty();
      this.createRootNode();

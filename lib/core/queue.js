...
class AsyncQueue extends EventEmitter {
  ...
  runChildNode(node) {
    this.runCommand(node, result => {
      ...
      // let childNodesInProcess = node.childNodes.some(childNode => {
      //   return childNode.done !== true;
      // });
      ...
      Logger.log(`childNodesInProcess: ${childNodesInProcess}; ${Logger.colors.green('→')} Completed command ${Logger.colors.light_green(node.name)} (${timeMs}ms)`);
      node.done = true;

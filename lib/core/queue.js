...
class AsyncQueue extends EventEmitter {
  ...
  runCommand(node, callback) {
    ...
    try {
      ...
      err.name = `Error while running "${node.name}" command: "${originalError.split('\n')[0]}"`;
      this.emit('error', err);
    }
  ...
  list() {
    return this.rootNode.children.map(function(item) {
      return item.name;
    });
  }

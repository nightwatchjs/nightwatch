...
class TreeNode {
  ...
  takeSnapshot() {
    if (!this.context.isTraceEnabled || !Utils.isTraceable(this.fullName)) {
      return;
    }

    const {name, module} = this.context.api.currentTest;

    this.snapShotPath = Utils.Snapshots.getFileName({
      testSuite: module,
      testCase: name,
      commandName: this.fullName,
      traceSettings: this.context.traceSettings,
      output_folder: this.context.settings.output_folder
    });

    this.context.api.saveSnapshot(this.snapShotPath);
  }
}

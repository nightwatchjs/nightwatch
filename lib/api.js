  ...
  function loadCustomCommands() {
    ...
    commandFiles.forEach(function(file) {
      if (path.extname(file) == '.js') {
        ...
        addCommand(name, command.command, client, client);
      }
    });
  };
      ...
      if (assertModule.hasOwnProperty(prop)) {
        ...
        (function(prop) {
          return function() {
            ...
            return Assertion.assert(passed, actual, expected, message, true);  
          };
        })(prop);
      }
    }
    
    loadAssertionFiles(client.assert, true);
    loadAssertionFiles(client.verify, false);
  };
  ...
  function loadAssertionFiles(parent, abortOnFailure) {
    ...
    for (var i = 0, len = commandFiles.length; i < len; i++) {
      if (path.extname(commandFiles[i]) == '.js') {
        ...
        var m = new Module();
        AbstractAssertion.call(m, abortOnFailure, client);

const STRATEGY_RECURSION = 'recursion';
...
class Element {
  ...
  get usingRecursion() {
    return this.locateStrategy == STRATEGY_RECURSION;
  }
  ...
  getRecursiveLookupElement() {
    ...
    if (lookupList.length > 1) {
      return new Element({
        selector: lookupList,
        locateStrategy: STRATEGY_RECURSION
      });
    }
  ...
  static createFromSelector(value, using) {
    ...
    if (using !== STRATEGY_RECURSION && typeof value === 'object') {
      definition = value;
    } else {
      definition = {
        selector: value

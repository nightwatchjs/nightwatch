...
function isValidAssertion(commandName) {
  return ['assert', 'verify', 'expect'].contains(commandName);
}
...
class Command {
  ...
  parseElementSelector(args) {
    ...
    let inputElement = possibleElementSelector && Element.fromSelector(possibleElementSelector);

    if (inputElement && inputElement.hasElementSelector()) {

      let getter = isSectionSelector ? this.getSection : this.getElement;
      let elementOrSection = getter(inputElement.selector);

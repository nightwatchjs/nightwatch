...
class RerunJsonReporter extends BaseReporter {
  ...
  static isRerunFailedEnabled(argv = {}) {
    if (process.env.NIGHTWATCH_RERUN_FAILED === 'true' || argv['rerun-failed']) {
      return true;
    }
  ...
  static getRerunFailedFile(rerun_report_file_path) {
    const jsonFile = path.resolve(process.env.NIGHTWATCH_RERUN_REPORT_FILE || rerun_report_file_path || '');
    ...
    if (!Utils.fileExistsSync(jsonFile)) {
      ...
      err.help = [
        `Try setting ${Logger.colors.cyan('rerun_report_file_path: "JSON-REPORTER-PATH"')} in nightwatch configuration`,
  ...
  writeReport() {
    ...
    return this.writeReportFile(filename, JSON.stringify(report), shouldCreateFolder, output_folder)
      .then(_ => {
        ...
        // Setting env varaible with rerunJsonReporter path.

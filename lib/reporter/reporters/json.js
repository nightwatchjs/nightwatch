...
class JsonReporter {
  ...
  static loadTemplate() {
    return new Promise((resolve, reject) => {
      if (JsonReporter.tmplData) {
        return resolve(JsonReporter.tmplData);
      }

      fs.readFile(JsonReporter.templateFile, (err, data) => {
        if (err) {
          return reject(err);
        }

        JsonReporter.tmplData = data.toString();
        resolve(JsonReporter.tmplData);
      });
    });
  }
  ...
  writeReport(moduleKey, data = {}) {
    ...
    return this.writeReportFile(filename, JSON.stringify(report), shouldCreateFolder, output_folder)
      .then(_ => {
        console.info(`Wrote report file to: ${filename}.`);
      });
  }
...
module.exports = (function() {
  ...
  return {
    write(results, options, callback) {
      let reporter = new JsonReporter(results, options);

...
module.exports = class GlobalReporter {
  ...
  beautifyStackTrace(err) {
    if (err && ['NightwatchAssertError'].includes(err.name)) {
      const parsedStack = stackTraceParser.parse(err.stack)[0];
      var currentLine = 1;
      require('fs').readFileSync(parsedStack.file, 'utf-8').split(/\r?\n/).forEach(function(line){
        if (line == "")
          return;
        if (currentLine === parsedStack.lineNumber) {
          console.log(colors.light_red("\t" + currentLine + " " + line));
        }
        else {
  ...
  print() {
    ...
    if (this.hasTestFailures()) {
      ...
      this.beautifyStackTrace(this.globalResults.lastError)

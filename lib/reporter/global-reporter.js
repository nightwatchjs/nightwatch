...
module.exports = class GlobalReporter {
  ...
  loadReporter() {
    ...
    if (Array.isArray(this.reporterFile)) {
      ...
      return this.loadFile(this.reporterFile);
    }
  }
  ...
  loadCustomReporter(reporterFile) {
    return Utils.fileExists(reporterFile)
      .then(customReporterExists => {
        if (!customReporterExists) {
          throw new Error(`The custom reporter file name "${reporterFile}" cannot be resolved.`);
        }

        const reporter = require(path.resolve(reporterFile));
  ...
  async writeReportToFile(globalResults) {
    ...
    try {
      ...
      const reporter = await this.loadReporter();

      if (Array.isArray(reporter)) {
        return Promise.all(reporter.map(reporterFile => this.writeReport(reporterFile, globalResults)));
      } else {
        return this.writeReport(reporter, globalResults);
      }
    } catch (err) {
  ...
  writeReport(reporter, globalResults) {
  ...
  getTestsFailedMessage() {
    ...
    let skippedSuitesCount = this.skippedSuites > 0 ? `\n   - ${colors.light_red(this.skippedSuites)} other test suites were aborted; ` : '';

    if (!hasCount) {
      let passedCount = Math.max(0, totalTests - this.globalResults.failed - this.globalResults.errors); // testcases passed

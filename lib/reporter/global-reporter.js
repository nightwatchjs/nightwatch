...
module.exports = class GlobalReporter {
  ...
  loadReporter() {
    const builtInReporter = ['junit', 'html'];

    // check if only one reporter is passed to Nightwatch client
    if (typeof this.reporterFile === 'string' || this.reporterFile instanceof String) {
      this.reporterFile = this.reporterFile.split(',').map((str) => str.replace(/\s/g, ''));
    }

    // In case of only one reporter present, append missing reporter to the array
    this.reporterFile = builtInReporter.reduce((acc, str) => {
      const found = this.reporterFile.find((item) => item === str);
    ...
    if (Array.isArray(this.reporterFile)) {
      return Promise.all(this.reporterFile.map((file) => this.loadFile(file)));
    }
  }
  ...
  async loadCustomReporter() {
    let customReporterModuleExists;
    let reporter;

    try {
      reporter = require(this.reporterFile);
      customReporterModuleExists = true;
    } catch (err) {
      customReporterModuleExists = false;
    }

    const customReporterExists = await Utils.fileExists(this.reporterFile);

    if (!customReporterExists && !customReporterModuleExists) {
      throw new Error(`The custom reporter "${this.reporterFile}" cannot be resolved.`);
    }

    if (!customReporterModuleExists) {
      reporter = require(path.resolve(this.reporterFile));
    }
    ...
    throw new Error('The reporter module must have a public ".write(results, options, [callback])" method defined which should return a Promise.');
  }
  ...
  async writeReportToFile(globalResults) {
    ...
    try {
      ...
      if (Array.isArray(reporters)) {
        ...
        return this.writeReport(reporters, globalResults);
      }
    } catch (err) {

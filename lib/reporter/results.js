...
module.exports = class Results {
  constructor(tests = [], opts, desiredCapabilities = {}) {
    ...
    this.sessionId = '';
    this.projectName = desiredCapabilities.projectName;
    this.buildName = desiredCapabilities.buildName;
  ...
  get export() {
    ...
    suiteResults.results.driverCapabilities = this.driverCapabilities;
  ...
  setTestStatus() {
    const currentTest = this.getCurrentTest();
    if (currentTest) {
      if (this.currentTestResult.errors === 0 && this.currentTestResult.failed === 0) {
        this.currentTestResult.status = 'pass';
      } else {
        this.currentTestResult.status = 'fail';
      }
    }
  }

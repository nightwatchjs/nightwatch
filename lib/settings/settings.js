...
class Settings {
   ...
   * @param {Object} [target]
   */
  static replaceEnvVariables(target, depth = 0, visited = new Set()) {
    // Maximum depth limit to prevent infinite recursion
    const MAX_DEPTH = 100;

    if (depth > MAX_DEPTH) {
      console.warn(`Max depth of ${MAX_DEPTH} exceeded`);
      return;
    }

    // Ensure the target is an object
    if (typeof target !== 'object' || target === null) {
      console.warn('Target is not an object:', target);
      return;
    }

    // Check for circular references
    if (visited.has(target)) {
      console.warn('Circular reference detected:', target);
      return;
    }
    visited.add(target);
    ...
    try {
      for (const key in target) {
        if (Object.prototype.hasOwnProperty.call(target, key)) {
          switch (typeof target[key]) {
            case 'object':
              console.log('Processing object at depth:', depth, target);
              this.replaceEnvVariables(target[key], depth + 1, visited);
              break;

            case 'string':
              target[key] = target[key].replace(/\${(\w+)\|?([^}]*)}/, (match, varName, defaultValue) => {
      ...
      // Optionally, handle specific types of errors if needed
    }

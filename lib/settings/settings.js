...
class Settings {
  ...
  inheritFromDefaultEnv() {
    ...
    let defaultEnv = this.baseSettings.test_settings['default'] || {};
  ...
  setStartSession() {
    if (this.settings.selenium.start_session !== undefined) {
      this.settings.start_session = this.settings.selenium.start_session;
    }
  ...
  readExternalGlobals() {
    if (this.settings.globals_path) {
      ...
      defaultsDeep(globals, externalGlobals);

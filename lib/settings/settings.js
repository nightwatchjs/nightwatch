...
class Settings {
  ...
  init(testEnv = null, settings = {}) {
    ...
    if (this.argv.timeout) {
      const timeout = parseInt(this.argv.timeout, 10);
      if (!isNaN(timeout)) {
        sortedSettings.globals.waitForConditionTimeout = timeout;
        sortedSettings.globals.retryAssertionTimeout = timeout;
      }
    }

var util = require('util');

module.exports = new (function() {
  var Settings = {
    log_timestamp : false,
    enabled : true,
    colors : true
  };
  
  var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep',
                'Oct', 'Nov', 'Dec'];
  
  function ConsoleColor() {
    var self = this;
    var mappings = {
      blue  : '0;34',
      light_blue  : '1;34',
    };
    this.background = new Background();
    
    this.foreground_colors = {};  
    this.foreground_colors['black'] = '0;30';
    this.foreground_colors['dark_gray'] = '1;30';
    this.foreground_colors['blue'] = '';
    this.foreground_colors['light_blue'] = '';
    this.foreground_colors['green'] = '0;32';
    this.foreground_colors['light_green'] = '1;32';
    this.foreground_colors['cyan'] = '0;36';
    this.foreground_colors['light_cyan'] = '1;36';
    this.foreground_colors['red'] = '0;31';
    this.foreground_colors['light_red'] = '1;31';
    this.foreground_colors['purple'] = '0;35';
    this.foreground_colors['light_purple'] = '1;35';
    this.foreground_colors['brown'] = '0;33';
    this.foreground_colors['yellow'] = '1;33';
    this.foreground_colors['light_gray'] = '0;37';
    this.foreground_colors['white'] = '1;37';
   
    this.background_colors = {};
    this.background_colors['black'] = '40';
    this.background_colors['red'] = '41';
    this.background_colors['green'] = '42';
    this.background_colors['yellow'] = '43';
    this.background_colors['blue'] = '44';
    this.background_colors['magenta'] = '45';
    this.background_colors['cyan'] = '46';
    this.background_colors['light_gray'] = '47';
    
    Object.keys(this.foreground_colors).forEach(function(k) {
      ConsoleColor.prototype[k.toLowerCase()] = function Foreground(text, background) {
        if (!Settings.colors) {
          return '';
        }
        var string = "\033[" + self.foreground_colors[k.toLowerCase()] + 'm';
        if (background != undefined) {
          string += background();
        }
        
        string += text + '\033[0m';
        return string;
      };
    });
    
    Object.keys(this.background_colors).forEach(function(k) {
      Background.prototype[k.toLowerCase()] = function Background(text) {
        if (!Settings.colors) {
          return '';
        }
        return "\033[" + self.background_colors[k.toLowerCase()] + 'm';
      };
    });
    return this;
  };
  
  function Background() { return this; };
  
  var colors = new ConsoleColor();
    
  /**
   * Print an object and its properties
   *  
   * @param {Object} obj
   */              
  function logObject(obj) {
    util.puts(util.inspect(obj, false, 3, true));
  };
  
  /**
   * Print the timestamp
   */
  function logTimestap() {
    if (Settings.log_timestamp) {
      return colors.white(timestamp()) + ' ';
    }
    return '';
  }
  
  /**
   * Print a various message
   * 
   * @param {String} type
   * @param {String} message
   * @param {Array} args
   */
  function logMessage(type, message, args) {
    if (!message || !Settings.enabled) {
      return;
    }
    
    var messageStr = '';
    switch (type) {
      case 'ERROR':
        messageStr = (Settings.colors && (colors.yellow(type, colors.background.dark_gray) + ' ') || '') + 
          logTimestap()  + colors.light_green(message);
        break;
      case 'INFO':
        messageStr = (Settings.colors && (colors.light_purple(type, colors.background.black) + ' ') || '') + 
          logTimestap() + colors.light_cyan(message);
        break;
      case 'LOG':
        messageStr = (Settings.colors(colors.white(type + ' ', colors.background.black) + ' ') || '') + 
          logTimestap() + colors.white(message);      
        break;  
      case 'WARN':
        messageStr = (Settings.colors(colors.light_green(type, colors.background.black) + ' ') || '') + 
          logTimestap() + colors.light_green(message);
        break;  
    }
    
    util.print(messageStr);
    
    if (args.length > 0) {
      var inlineArgs = [];
      args.forEach(function(item) {
        if (Object.prototype.toString.call(item) === '[object Object]' && Object.keys(item).length > 0) {
          if (inlineArgs.length) {
            console.log.apply(console, inlineArgs);
            inlineArgs = [];
          }
          logObject(item);  
        } else {
          inlineArgs.push(item);
        }   
      });
      if (inlineArgs.length) {
        util.print(' ');
        console.log.apply(console, inlineArgs);
        inlineArgs = [];
      }
    } else {
      util.print('\n');  
    }
  };
  
  /**
   * Retrieve the severity level prefix
   *  
   * @param {String} tag
   * @param {String} severity
   * @return {String}
   */  
  function getPrefix(tag, severity) {
    var levels = ['EMERG', 'ALERT', 'CRITICAL', 'ERROR', 'WARNING', 'NOTICE', 'INFO', 'DEBUG'];
    
    return tag + ' ' + levels[severity] +' - ';
  }  
  ////////////////////////////////////////////////
  // Date formatting helpers
  ///////////////////////////////////////////////
  function getDate() {
    var now = new Date();
    return [now.toLocaleDateString(), now.toLocaleTimeString()].join(' ');
  }
  
  function pad(n) {
    return n < 10 ? '0' + n.toString(10) : n.toString(10);
  }
  
  // 26 Feb 16:19:34
  function timestamp() {
    var d = new Date();
    var time = [pad(d.getHours()),
                pad(d.getMinutes()),
                pad(d.getSeconds())].join(':');
    return [d.getDate(), months[d.getMonth()], time].join(' ');
  };
  
  /**
   * Log an INFO message
   *  
   * @param {String} message
   */
  this.info = function(message) {
    var args = Array.prototype.slice.call(arguments, 1);
    logMessage("INFO", message, args);
  };
  
  /**
   * Log a regular LOG message
   *  
   * @param {String} message
   */
  this.log = function(message) {
    var args = Array.prototype.slice.call(arguments, 1);
    logMessage("LOG", message, args);
  };
  
  /**
   * Log a WARN message
   *  
   * @param {String} message
   */
  this.warn = function(message) {
    var args = Array.prototype.slice.call(arguments, 1);
    logMessage("WARN", message, args);
  };
  
  /**
   * Log an ERROR message
   *  
   * @param {String} message
   */
  this.error = function(message) {
    var args = Array.prototype.slice.call(arguments, 1);
    logMessage("ERROR", message, args);
  };
  
  /**
   * Disable logging completely
   */
  this.disable = function() {
    Settings.enabled = false;
  };
  
  /**
   * Disable the coloring of the output
   */
  this.disableColors = function() {
    Settings.colors = false;
  };
  
  /**
   * Enable logging
   */
  this.enable = function() {
    Settings.enabled = true;
  };
  
  /**
   * Check if logging is enabled
   * 
   * @return {Boolean}
   */
  this.isEnabled = function() {
    return Settings.enabled;
  };
  
  this.colors = colors;
  
})();

    ...
    Walk.readPaths(fullPaths, function (err, modules) {
      ...
      if (joinedPaths === fullPaths.length) {
        ...
        (function runNextModule() {
          ...
          runTestModule(modulePath, fullPaths, opts, additionalOpts).then(function(testResults) {
            if (modulePaths.length) {
              ...
              if (opts.output) {
                reporter.printTotalResults(globalResults, testResults, globalStartTime);
              }

              reporter.save().then(function() {
                reporter.globalReporter.call(opts.globals, globalResults, function() {
                  finishCallback(null, globalResults);
                });
              }, function(err) {

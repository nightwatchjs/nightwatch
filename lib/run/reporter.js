...
Reporter.prototype.get = function() {
  ...
  if (fs.existsSync(fileName)) {
    ...
    throw new Error('The reporter module must have a public `save` method defined.');
  }
...
Reporter.prototype.globalReporter = function(globals) {
  ...
  return Utils.makeFnAsync(2, reporterFn);
};

Reporter.prototype.isDisabled = function() {
  return this.output_folder === false;
};
...
Reporter.prototype.printTotalResults = function(modulekeys, results) {
  ...
  if (this.globalResults.passed > 0 && this.globalResults.errors === 0 && this.globalResults.failed === 0) {
    ...
    var skippedTestcases = this.testResults.steps;
    if (skippedTestcases && skippedTestcases.length > 0) {
      var keys = skippedTestcases.map(function(e) {
        return Logger.colors.cyan('"' + e + '"');
      });
      var plural = skippedTestcases.length > 1 ? 's' : '';
      skipped += 'Testcase' + plural + ' ' + keys.join(', ') + ' skipped.';
    }
    var errorsMsg = '';
    ...
    ' assertions failed, ' + Logger.colors.green(this.globalResults.passed) + ' passed', '(' + Utils.formatElapsedTime(elapsedTime) + ')');
    console.log(skipped);
  }
};

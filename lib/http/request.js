...
const {USER_EVENTS: {LogCreated}, NightwatchEventHub} = require('../runner/eventHub.js');
...
class HttpRequest extends EventEmitter {
  ...
  onRequestComplete(result, response) {
    ...
    if (NightwatchEventHub.isAvailable && NightwatchEventHub.runner !== 'cucumber') {
      NightwatchEventHub.emit({
        eventName: LogCreated,
        hook_type: Logger.collectCommandOutput()
      });
    }

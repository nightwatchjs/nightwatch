...
class HttpRequest extends EventEmitter {
  ...
  static getHttpsKeepAliveAgent(keepAliveMsecs, maxSockets) {
  ...
  createHttpOptions(options) {
    ...
    if (this.httpOpts.keep_alive) {
      ...
      if (enabled) {
        reqOptions.agent = reqOptions.port === 443 ? HttpRequest.getHttpsKeepAliveAgent(keepAliveMsecs, 1) : HttpRequest.getHttpsKeepAliveAgent(keepAliveMsecs, 1);
      }
    }

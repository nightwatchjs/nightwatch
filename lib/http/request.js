...
class HttpRequest extends EventEmitter {
  ...
  isRedacted(data, reqOptions) {
    if (reqOptions?.method !== 'POST'){
      return false;
    }
    if (!reqOptions?.path.endsWith('/value')){
      return false;
    }
    for (const value of Object.values(data)) {
      if (typeof(value) === 'string' && value.startsWith(Key.NULL)) {
        return true;
      }
    }
  ...
  setOptions(options) {
    ...
    this.redact = this.isRedacted(options.data, this.reqOptions);

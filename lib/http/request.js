...
let __httpsKeepAliveAgent__ = null;
...
class HttpRequest extends EventEmitter {
  ...
  static getSecureKeepAliveAgent({keepAliveMsecs, maxSockets}) {
    if (!__httpsKeepAliveAgent__) {
      __httpsKeepAliveAgent__ = new https.Agent({
        keepAlive: true,
        keepAliveMsecs,
        maxSockets
      });
    ...
    return __httpsKeepAliveAgent__; 
  }
  ...
  setkeepAliveAgent(reqOptions) {
    if (this.httpOpts.keep_alive) {
      ...
      if (enabled) {
        reqOptions.agent = reqOptions.port === 443 ? HttpRequest.getSecureKeepAliveAgent({keepAliveMsecs, maxSockets: 1}) : HttpRequest.getHttpKeepAliveAgent({keepAliveMsecs, maxSockets: 1});
      }
    }
  }
  ...
  createHttpOptions(options) {
    ...
    this.setkeepAliveAgent(reqOptions);
